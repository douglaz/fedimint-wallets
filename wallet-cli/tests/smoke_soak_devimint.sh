...
#
# Run (two-fed harness; first follow docs/devimint-runbook.md §1's exact-pin patch/release
# build, which exports FEDIMINT_WORKTREE):
#   set -euo pipefail
...
#       unset "$variable"
#     done
#     export WALLET_CLI_BIN="$WALLETS_REPO/target-nix/release/wallet-cli"
#     export WALLETD_BIN="$WALLETS_REPO/target-nix/release/walletd"
...
#       --exec bash "$WALLETS_REPO/wallet-cli/tests/smoke_soak_devimint.sh"
#   '
set -euo pipefail
...
for f in "$WALLET_CLI" "$WALLETD"; do
  if [[ ! -x "$f" ]]; then
    ...
    echo '  run_exact_cargo build --release --locked --target-dir "$WALLETS_REPO/target-nix" -p wallet-daemon -p wallet-cli' >&2
    exit 1
  fi
done
