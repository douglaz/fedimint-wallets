...
#
# NOT part of the rb-lite gate (needs a LIVE two-fed devimint; run by hand). First follow
# docs/devimint-runbook.md §1's exact-pin patch/release build, which exports FEDIMINT_WORKTREE.
# The same harness as smoke_tick/smoke_probe supplies $FED_B_INVITE and connects/pegs the shared
# LDK gateway. REBUILD wallet-cli into this repo's target-nix BEFORE running:
#   set -euo pipefail
...
#
# Run inside smoke_tick_devimint.sh's full two-federation invocation (which exports
# FM_ENABLE_MODULE_MINT=1, FM_ENABLE_MODULE_WALLET=1, and FM_ENABLE_MODULE_LNV2=1) with
# FED_B_INVITE set; see that header for the full bring-up.
set -euo pipefail
