...
# gateway. Follow docs/devimint-runbook.md §1's exact-pinned two-fed patch/release procedure
# (it stands up fed B under `--num-feds 2`, connects + funds the LDK gateway on B, and exports
# $FED_B_INVITE), or stand up B yourself and `gateway-ldk connect-fed "$FED_B_INVITE"`.
#
...
#
#   # 3. Bring up fed A + fed B from the exact-pinned release binary, then run this inside the exec:
...
#       --num-feds 2 dev-fed \
#       --exec bash "$WALLETS_REPO/wallet-cli/tests/smoke_crash_move_devimint.sh"
#   '
#
