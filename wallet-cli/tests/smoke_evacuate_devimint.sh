...
#   # 2. Build wallet-cli (DEBUG — the force-shutdown seam is compiled out of --release):
#   set -euo pipefail
...
#       --num-feds 2 dev-fed \
#       --exec bash "$WALLETS_REPO/wallet-cli/tests/smoke_evacuate_devimint.sh"
#   '
#
...
if [[ ! -x "$WALLET_CLI" ]]; then
  ...
  echo 'Follow docs/devimint-runbook.md §1 to export FEDIMINT_WORKTREE, then run:' >&2
