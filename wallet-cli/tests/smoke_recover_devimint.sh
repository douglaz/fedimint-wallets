...
#   # 2. Build the wallet binaries FRESH (a stale binary tests old code — the #1 false pass):
#   set -euo pipefail
...
#     "$DEVIMINT_BIN" --link-test-dir "$FEDIMINT_WORKTREE/target-nix/devimint" --num-feds 1 dev-fed \
#       --exec bash "$WALLETS_REPO/wallet-cli/tests/smoke_recover_devimint.sh"'
#
