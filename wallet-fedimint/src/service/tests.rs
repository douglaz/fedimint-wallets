...
use wallet_core::{
    Action, EvacFeeCap, EvacuationQuoteSample, EvacuationRefusalEvidence, Journal, Occurrence,
    OperationId, OperationKind, OperationStatus, PerformOutcome, ReasonCode, RefusalDiagnostics,
};
...
#[derive(Default)]
struct StructuralRefusalExecutor {
    calls: AtomicUsize,
}

struct EvidenceStructuralRefusalExecutor {
    calls: AtomicUsize,
    evidence: EvacuationRefusalEvidence,
}

struct HeldStructuralRefusalExecutor {
    calls: AtomicUsize,
    evidence: EvacuationRefusalEvidence,
    started: Notify,
    release: Notify,
...
/// A two-phase evacuation executor for the replacement hand-off: the first
/// execution produces the durable structural marker, while the replacement
/// child remains live until the test explicitly releases it.
#[derive(Default)]
struct ReplacementHandoffExecutor {
    calls: AtomicUsize,
    performed: Mutex<Vec<Intent>>,
    old_started: Notify,
...
#[async_trait]
impl Executor for StructuralRefusalExecutor {
    async fn perform(&self, _intent: &Intent) -> Result<PerformOutcome, ExecError> {
        ...
        Err(ExecError::StructuralEvacuationRefusal(
...
#[async_trait]
impl Executor for EvidenceStructuralRefusalExecutor {
    async fn perform(&self, _intent: &Intent) -> Result<PerformOutcome, ExecError> {
        self.calls.fetch_add(1, Ordering::SeqCst);
        Err(ExecError::StructuralEvacuationRefusal(
            self.evidence.clone(),
...
#[async_trait]
impl Executor for HeldStructuralRefusalExecutor {
    async fn perform(&self, _intent: &Intent) -> Result<PerformOutcome, ExecError> {
        self.calls.fetch_add(1, Ordering::SeqCst);
...
#[async_trait]
impl Executor for ReplacementHandoffExecutor {
    async fn perform(&self, intent: &Intent) -> Result<PerformOutcome, ExecError> {
        let call = self.calls.fetch_add(1, Ordering::SeqCst);
        self.performed
            .lock()
            .expect("replacement performed-intent lock")
            .push(intent.clone());
        if call == 0 {
            let Action::Evacuate {
                amount,
                ...
                panic!("first replacement-handoff execution must be an evacuation with a cap");
            };
            self.old_started.notify_one();
            return Err(ExecError::StructuralEvacuationRefusal(
                EvacuationRefusalEvidence {
                    cap_components: cap,
                    requested_net: amount,
                    source_spendable: Msat(
                        amount.0.saturating_add(cap.at(amount).0).saturating_add(1),
                    ),
                    low: EvacuationQuoteSample {
                        delivered_net: Msat(10),
                        total_fee: Msat(cap.at(Msat(10)).0.saturating_add(1)),
                        fee_cap: cap.at(Msat(10)),
                    },
                    high: EvacuationQuoteSample {
                        delivered_net: amount,
                        total_fee: Msat(cap.at(amount).0.saturating_add(1)),
                        fee_cap: cap.at(amount),
                    },
                    diagnostic: "controlled structural evacuation refusal".to_owned(),
                    measured_at_ms: 2,
                },
            ));
        }
...
#[tokio::test]
async fn ambiguous_structural_reset_wakes_scheduler_after_marker_is_reread() {
    let (service, journal) = fixture(Arc::new(SlowExecutor::default())).await;
    let client = service.client();
    let mut wake = service.policy_wake.clone();
    let key = IdempotencyKey("evac:ambiguous-marker-wake".into());
    let cap = EvacFeeCap {
        base_msat: Msat(10),
        bps: 100,
    };
    journal
        ...
        .await
        .expect("seed executing evacuation");
    let evidence = EvacuationRefusalEvidence {
        cap_components: cap,
        requested_net: Msat(100),
        source_spendable: Msat(100),
        low: EvacuationQuoteSample {
        ...
        diagnostic: "measured structural refusal".into(),
        measured_at_ms: 1,
    };
    let stale_facts = client
        .issue_balance_facts_token()
        .await
        .expect("facts before ambiguous retryable reset");
    let stale_plan = client
        .issue_tick_plan_token()
        .await
        .expect("plan before ambiguous retryable reset");
    journal.fail_after_next_retryable_reset_for_test();
    client
        .journal_transition(
            key.clone(),
            JournalTransition::ResetRetryable {
                expected_attempt: 0,
                structural_refusal: Some(evidence.clone()),
            },
        )
        .await
        .expect_err("post-commit reset fault must reach the caller");
    tokio::time::timeout(std::time::Duration::from_secs(1), wake.changed())
        .await
        .expect("confirmed durable marker wakes scheduler")
        .expect("wake sender remains live");
    assert_eq!(
        journal
            .get(&key)
    ...
    assert_eq!(
        client
            .issue_balance_facts_token()
            .await
            .expect("ambiguous reset is a scoped, not global, fact bump")
            .generations,
        BTreeMap::from([(fed(1), 1), (fed(2), 1)]),
        "the reset can only have changed the evacuation endpoints"
    );
    let touched = one_msat_funding(
        "move:touched-after-ambiguous-retryable-reset",
        fed(1),
        fed(3),
        Occurrence(611),
    );
    let independent = one_msat_funding(
        "move:independent-after-ambiguous-retryable-reset",
        fed(4),
        fed(5),
        Occurrence(611),
    );
    let report = client
        .commit_tick(
            TickRound::for_test(vec![touched.clone(), independent.clone()], 0, stale_plan),
            BTreeMap::from([
                (fed(1), Msat(1)),
                (fed(3), Msat(0)),
                (fed(4), Msat(1)),
        ...
        .await
        .expect("known retryable-reset ambiguity is scoped");
    assert!(
        report
            .refused
            .iter()
...
#[tokio::test]
async fn unchanged_structural_marker_waits_for_reconcile_but_a_later_policy_update_wakes() {
    let (service, journal) = fixture(Arc::new(SlowExecutor::default())).await;
    let client = service.client();
    let key = IdempotencyKey("evac:unchanged-marker-no-wake".into());
    let cap = EvacFeeCap {
        base_msat: Msat(10),
        bps: 100,
    };
    let mut policy = client.get_policy().await.expect("policy");
    policy.evac_fee_base_msat = cap.base_msat;
    policy.evac_fee_bps = cap.bps;
    client.put_policy(policy).await.expect("set equal cap");
    let mut wake = service.policy_wake.clone();
    ...
    client
        ...
        .await
        .expect("equal-cap marker lands");
    assert!(
        tokio::time::timeout(std::time::Duration::from_millis(25), wake.changed())
            .await
            .is_err(),
        "an unchanged marker waits for the normal reconciliation interval"
    );
    let pending = journal
        .get(&key)
        .await
    ...
    assert_eq!(pending.evacuation_refusal, Some(evidence));

    let mut increased = client.get_policy().await.expect("current policy");
    increased.evac_fee_base_msat = Msat(11);
    client
        .put_policy(increased)
        .await
        .expect("qualifying later policy update");
    tokio::time::timeout(std::time::Duration::from_secs(1), wake.changed())
        .await
        .expect("PutPolicy wakes the scheduler")
        .expect("wake sender remains live");
...
#[tokio::test]
async fn ordinary_reconcile_and_driver_finished_do_not_redrive_planner_owned_marker() {
    let executor = Arc::new(StructuralRefusalExecutor::default());
    let (service, journal) = fixture(executor.clone()).await;
    let client = service.client();
    let key = IdempotencyKey("evac:planner-owned-marker".into());
    let cap = EvacFeeCap {
        base_msat: Msat(10),
        bps: 100,
    };
    journal
        ...
        .await
        .expect("seed evacuation");
    client.reconcile_durable().await.expect("first reconcile");
    loop {
        if journal
            .get(&key)
            .await
            .expect("read marker")
            .is_some_and(|intent| intent.evacuation_refusal.is_some())
        {
            break;
        }
        tokio::task::yield_now().await;
    ...
    client
        .reconcile_durable()
        .await
        .expect("ordinary reconcile leaves marker to planner");
    // Shutdown is the deterministic zero-driver barrier: it drains every driver and the actor
    // handoff before the assertion, unlike a scheduler yield that may run before DriverFinished.
    service.shutdown().await.expect("shutdown");
    assert_eq!(
        executor.calls.load(Ordering::SeqCst),
        1,
        "neither reconcile nor DriverFinished may launch a marked Pending intent"
    );
    assert!(
        journal
            .get(&key)
            ...
            .evacuation_refusal
            .is_some(),
        "the planner handoff remains durable"
    );
}

/// A recovery-only scheduler pass has committed not to plan. It must therefore claim the old
/// marker rather than park it, but a renewed structural refusal is not new planning information
/// until a later healthy pass can capture it.
#[tokio::test]
async fn recovery_only_claim_clears_old_marker_remarks_without_an_immediate_wake() {
    let (parent, evidence, new_cap) = qualifying_marker(fed(71), fed(72), Occurrence(701));
    let executor = Arc::new(EvidenceStructuralRefusalExecutor {
    ...
    policy.evac_fee_base_msat = new_cap.base_msat;
    policy.evac_fee_bps = new_cap.bps;
    client
        .put_policy(policy)
        .await
    ...
    journal
        .upsert(&parent)
        .await
        .expect("seed qualifying planner marker");
    let mut wake = service.policy_wake.clone();
    wake.borrow_and_update();
    ...
    assert_eq!(
        client
            .reconcile()
            .await
            .expect("healthy planner pass captures old marker")
            .redriven,
        0
    );
    assert_eq!(
        client
            .reconcile_recovery_only_cycle()
            .await
            .expect("recovery-only pass claims old marker")
            .redriven,
        1
    );
    tokio::time::timeout(std::time::Duration::from_secs(1), async {
        loop {
            let current = journal
                ...
                .await
                .expect("read renewed parent")
                .expect("recovery preserves parent row");
            if executor.calls.load(Ordering::SeqCst) == 1
                && current.status == IntentStatus::Pending
                && current.evacuation_refusal == Some(evidence.clone())
            {
                break;
            }
            tokio::task::yield_now().await;
    ...
    .await
    .expect("recovery claim clears old evidence before the driver writes renewed evidence");
    assert!(
        tokio::time::timeout(std::time::Duration::from_millis(25), wake.changed())
            .await
            .is_err(),
        "the recovery claim's one-shot suppression prevents a marker wake/tight scheduler loop"
    );
    assert_eq!(
        client
            .reconcile()
            .await
            .expect("later healthy pass recaptures renewed marker")
            .redriven,
        0,
        "a healthy planner pass must capture, not re-drive, renewed evidence"
    );
    service.shutdown().await.expect("shutdown");
}

/// A scheduler reconciliation owns the exact rows it skipped. Durable-only recovery may observe
/// those rows between scheduler cycles, but cannot replace the snapshots or release the marker
/// before the next `ReconcileDecide` gets its full-parent CAS turn.
#[tokio::test]
async fn reconcile_decide_parks_exact_markers_across_durable_recovery_then_redrives_only_cas_match()
{
    let executor = Arc::new(SlowExecutor::default());
    let (service, journal) = fixture(executor.clone()).await;
    let client = service.client();
    let (first, first_evidence, cap) = qualifying_marker(fed(1), fed(2), Occurrence(620));
    let (second, second_evidence, _) = qualifying_marker(fed(3), fed(4), Occurrence(621));
    journal
        .upsert(&first)
        .await
        .expect("seed first marked parent");
    journal
        .upsert(&second)
        .await
        .expect("seed second marked parent");
    let mut policy = client.get_policy().await.expect("read policy");
    policy.evac_fee_base_msat = cap.base_msat;
    policy.evac_fee_bps = cap.bps;
    client
        .put_policy(policy)
        .await
        .expect("install cap that qualifies both markers");
    let mut wake = service.policy_wake.clone();
    wake.borrow_and_update();
    ...
    client
        .reconcile()
        .await
        .expect("first scheduler reconcile captures its skipped parents");
    assert_eq!(
        executor.calls.load(Ordering::SeqCst),
        0,
        "a captured planner marker starts no recovery driver"
    );
    for (key, evidence) in [
        (first.idempotency_key.clone(), first_evidence.clone()),
        (second.idempotency_key.clone(), second_evidence.clone()),
    ] {
        assert_eq!(
            journal
                .get(&key)
                .await
                .expect("read parked parent")
                .expect("parent remains")
                .evacuation_refusal,
            Some(evidence),
            "the scheduler left its exact parked marker durable"
        );
    }
    ...
    client
        .reconcile_durable()
        .await
        .expect("intervening durable recovery");
    assert_eq!(
        journal
            .get(&first.idempotency_key)
            .await
            .expect("read first after durable recovery")
            .expect("first remains")
            .evacuation_refusal,
        Some(first_evidence),
        "durable recovery neither releases nor overwrites parked snapshots"
    );

    // Change every durable identity field comparison needs to notice. Its old snapshot must now
    // be a harmless exact-CAS miss, rather than clearing this newly written marker.
    let mut changed_second = journal
        .get(&second.idempotency_key)
        .await
        .expect("read second for concurrent replacement")
        .expect("second remains");
    changed_second.reason = ReasonCode::Unhealthy;
    journal
        .upsert(&changed_second)
        .await
        .expect("replace second full parent");

    let second_report = client
        .reconcile()
        .await
        .expect("next scheduler reconcile drains prior snapshots before scanning");
    assert_eq!(second_report.redriven, 1, "{second_report:#?}");
    assert_eq!(
        journal
            .get(&first.idempotency_key)
            .await
            .expect("read released first")
            .expect("first remains pending")
            .evacuation_refusal,
        None,
        "the unchanged full parent was released for ordinary recovery"
    );
    assert_eq!(
        journal
            .get(&second.idempotency_key)
            .await
            .expect("read changed second")
            .expect("second remains pending")
            .evacuation_refusal,
        Some(second_evidence),
        "the changed full parent is an exact-CAS miss and retains its marker"
    );
    tokio::time::timeout(std::time::Duration::from_secs(1), async {
        while executor.calls.load(Ordering::SeqCst) != 1 {
            tokio::task::yield_now().await;
    ...
    .await
    .expect("only the released parent receives a driver");
    assert!(
        tokio::time::timeout(std::time::Duration::from_millis(25), wake.changed())
            .await
            .is_err(),
        "parking and full-parent release do not synthesize a policy wake"
    );
    service.shutdown().await.expect("shutdown");
}

/// The bounded one-child handoff must survive a FAILED durable pass.
///
/// `reconcile` selects the offer AFTER the durable scan but BEFORE propagating that scan's error,
/// so a marker the failed pass already captured still receives its next-cycle full-parent release.
/// Deferring the selection to the next SUCCESSFUL reconciliation would silently double the bound a
/// structural marker waits before it returns to ordinary retry.
#[tokio::test]
async fn a_marker_captured_by_a_failed_reconcile_is_released_by_the_next_one() {
    let (service, journal) = fixture(Arc::new(SlowExecutor::default())).await;
    let client = service.client();
    let (parent, evidence, cap) = qualifying_marker(fed(1), fed(2), Occurrence(640));
    journal.upsert(&parent).await.expect("seed marked parent");
    // `pending()` scans Pending before Executing, so this row is processed AFTER the marker: the
    // injected status-write fault therefore fails the pass only once it has already captured.
    let mut stuck = agent_funding_intent(fed(3), fed(4), Msat(100), Occurrence(640));
    stuck.status = IntentStatus::Executing;
    journal.upsert(&stuck).await.expect("seed an executing row");
    let mut policy = client.get_policy().await.expect("read policy");
    policy.evac_fee_base_msat = cap.base_msat;
    policy.evac_fee_bps = cap.bps;
    client
        .put_policy(policy)
        .await
        .expect("install a cap that qualifies the marker");

    journal.fail_after_next_status_write_for_test();
    let error = client
        .reconcile()
        .await
        .expect_err("the durable scan fails after it captured the marker");
    assert!(
        matches!(
            &error,
            ServiceError::Storage(message)
                if message.contains("injected error after durable intent status write")
        ),
        "{error:?}"
    );
    assert_eq!(
        journal
            ...
            .evacuation_refusal,
        Some(evidence.clone()),
        "the failed pass releases nothing itself"
    );

    let report = client
        .reconcile()
        .await
        .expect("the next scheduler reconcile drains the offer the failed pass made");
    assert_eq!(
        journal
            .get(&parent.idempotency_key)
            .await
            .expect("read released parent")
            .expect("parent remains pending")
            .evacuation_refusal,
        None,
        "the marker captured by the failed pass got its bounded next-cycle release"
    );
    assert_eq!(
        report.redriven, 2,
        "the released parent and the normalized row both regain owners: {report:#?}"
    );
    service.shutdown().await.expect("shutdown");
}

#[tokio::test]
async fn commit_tick_marker_clear_fault_terminalizes_an_already_started_tick() {
    let (service, journal) = fixture(Arc::new(SlowExecutor::default())).await;
    let client = service.client();
    let (parent, evidence, cap) = qualifying_marker(fed(1), fed(2), Occurrence(622));
    journal.upsert(&parent).await.expect("seed marked parent");
    let mut policy = client.get_policy().await.expect("read policy");
    policy.evac_fee_base_msat = cap.base_msat;
    policy.evac_fee_bps = cap.bps;
    client.put_policy(policy).await.expect("qualifying policy");
    let token = client.issue_tick_plan_token().await.expect("plan token");
    let facts = client
        .issue_balance_facts_token()
        .await
        .expect("balance facts");
    let mut round = TickRound::for_test(Vec::new(), 1, token);
    round.marker_disposition = Some(EvacuationMarkerDisposition {
        parent: parent.clone(),
    });
    let tick_key = IdempotencyKey("tick:marker-clear-fault".into());
    journal
        .record_tick_started(&tick_key, Occurrence(0), 1)
        .await
        .expect("scheduler starts audit row");
    journal.fail_before_next_marker_clear_for_test();
    let error = client
        .commit_tick_with_facts(round, BTreeMap::new(), facts, Some(tick_key.clone()))
        .await
        .expect_err("marker-clear storage fault returns to scheduler");
    assert!(
        error
            .to_string()
            .contains("injected pre-marker-clear fault"),
        "{error}"
    );
    assert_eq!(
        journal
            ...
            .await
            .expect("read tick audit")
            .expect("started row survives")
            .status,
        OperationStatus::Failed,
        "a post-Started marker fault must not leak a Started tick"
    );
    assert_eq!(
        journal
            .get(&parent.idempotency_key)
            ...
            .evacuation_refusal,
        Some(evidence),
        "the failed clear leaves the exact durable marker intact"
    );
    service.shutdown().await.expect("shutdown");
}

#[tokio::test]
async fn policy_superseded_commit_retains_its_exact_marker_disposition_before_returning() {
    let (service, journal) = fixture(Arc::new(SlowExecutor::default())).await;
    let client = service.client();
    let (parent, _evidence, cap) = qualifying_marker(fed(1), fed(2), Occurrence(623));
    journal.upsert(&parent).await.expect("seed exact marker");
    let mut policy = client.get_policy().await.expect("read policy");
    policy.evac_fee_base_msat = cap.base_msat;
    policy.evac_fee_bps = cap.bps;
    client.put_policy(policy).await.expect("qualifying policy");
    ...
    let mut superseding_policy = client.get_policy().await.expect("current policy");
    superseding_policy.max_fee = Msat(superseding_policy.max_fee.0 + 1);
    client
        .put_policy(superseding_policy)
        .await
        .expect("supersede the planned authority");
    let tick_key = IdempotencyKey("tick:marker-policy-superseded".into());
    journal
        .record_tick_started(&tick_key, Occurrence(0), 1)
        .await
        .expect("scheduler started audit row");
    let report = client
        .commit_tick_with_facts(round, BTreeMap::new(), facts, Some(tick_key))
        .await
        .expect("policy supersession is a soft commit refusal");
    assert!(
        report.accepted.is_empty() && report.refused.is_empty(),
        "{report:#?}"
    );
    assert_eq!(
        journal
            ...
            .expect("parent remains")
            .evacuation_refusal,
        parent.evacuation_refusal,
        "a stale authority rejection must not clear an unvalidated marker disposition"
    );
    let recaptured = client
        .reconcile()
        .await
        .expect("the next healthy reconciliation recaptures the retained disposition marker");
    assert_eq!(recaptured.redriven, 0, "{recaptured:#?}");
    assert_eq!(
        journal
            .get(&parent.idempotency_key)
            .await
            .expect("read marker after recapture"),
        Some(parent),
        "the retained disposition marker remains durable for a valid later round"
    );
    service.shutdown().await.expect("shutdown");
}
...
/// br-n8o: the only way a structural evacuation marker becomes a new
/// evacuation is the actor-planned, actor-committed exchange.  Keep its child
/// blocked in `perform` long enough to prove the old holder was atomically
/// retired rather than briefly coexisting with a second reservation or goal.
#[tokio::test]
async fn structural_evacuation_marker_is_atomically_replaced_by_one_fresh_planned_child() {
    let executor = Arc::new(ReplacementHandoffExecutor::default());
    let (service, journal) = fixture(executor.clone()).await;
    let client = service.client();
    let old_occurrence = Occurrence(610);
    let child_occurrence = Occurrence(old_occurrence.0 + 1);
    let amount = Msat(200_000);
    ...
    // Put A before the old executor can refuse, so the marker's evidence and
    // the performed intent prove which policy governed the first attempt.
    let mut policy = client.get_policy().await.expect("read initial policy");
    policy.per_fed_cap = Msat(1_000_000);
    policy.spending_target = Msat(0);
    policy.standby_target = Msat(0);
    policy.evac_fee_base_msat = cap_a.base_msat;
    policy.evac_fee_bps = cap_a.bps;
    client
        .put_policy(policy.clone())
        .await
        .expect("install cap A before old evacuation");

    let old_key = evacuation_key(fed(1), fed(2), old_occurrence);
    let old_action = Action::Evacuate {
        from: fed(1),
        to: fed(2),
        amount,
        fee_cap: cap_a.at(amount),
        gateway: None,
    ...
    client
        .decide_op(agent_request(
            ...
            ReasonCode::ShutdownNotice,
            old_occurrence,
            BTreeMap::from([(fed(1), Msat(1_000_000)), (fed(2), Msat(0))]),
        ))
        .await
        .expect("admit old agent evacuation");
    tokio::time::timeout(
        std::time::Duration::from_secs(1),
        executor.old_started.notified(),
    )
    .await
    .expect("old evacuation reaches the controlled refusal");

    let old_marker = tokio::time::timeout(std::time::Duration::from_secs(1), async {
        loop {
            let marker = journal
                .get(&old_key)
                .await
                ...
                .filter(|intent| {
                    intent.status == IntentStatus::Pending && intent.evacuation_refusal.is_some()
                });
    ...
    .await
    .expect("driver persists a pending structural marker");
    let evidence = old_marker
        .evacuation_refusal
        .clone()
        .expect("pending old evacuation carries refusal evidence");
    assert_eq!(evidence.cap_components, cap_a);
    let performed_old = executor
        .performed
        .lock()
        .expect("performed intents")
        .first()
        .cloned()
        .expect("old executor records its performed intent");
    assert_eq!(performed_old.idempotency_key, old_key);
    assert_eq!(performed_old.action, old_action);
    assert_eq!(
        performed_old.evacuation_refusal, None,
        "the executor ran only after PutPolicy(A), before it made the marker"
    );

    // B is component-wise monotone and gains real integer effective room at
    // both recorded samples.  Reconcile must leave this qualifying marker for
    // the planner rather than re-drive the old key.
    policy.evac_fee_base_msat = cap_b.base_msat;
    policy.evac_fee_bps = cap_b.bps;
    client
        .put_policy(policy)
        .await
        .expect("install monotone effective cap B");
    assert!(wallet_core::evacuation_cap_qualifies_replacement(
        &evidence, cap_b
    ));
    let reconcile = client
        .reconcile()
        .await
        .expect("qualifying marker receives planner ownership");
    assert_eq!(
        reconcile.redriven, 0,
        "marker must not receive an old-key driver"
    );
    tokio::time::timeout(std::time::Duration::from_secs(1), async {
        loop {
            if registry_size(&client).await == 0 {
                return;
            }
            tokio::task::yield_now().await;
        }
    ...
    .await
    .expect("old driver finishes before the replacement plan");

    let round = client
        .decide_tick_round(ProbeFacts {
            probes: vec![(fed(1), dying_probe(300_000)), (fed(2), healthy_probe(0))],
            occurrence: child_occurrence,
            now_ms: 2,
            price_routes: false,
            blocked: reconcile.blocked,
            admission_snapshot: reconcile.admission_snapshot,
        })
        .await
        .expect("plan replacement from fresh unhealthy/healthy probe facts");
    let replacement = round
        .replacement
        .as_ref()
        .expect("qualifying marker produces an actor-owned replacement");
    assert!(
        round.decisions.is_empty(),
        "the shadow round contains no ordinary work: {:#?}",
        round.decisions
    );
    let fresh = replacement.fresh.clone();
    let child_key = replacement.fresh.idempotency_key.clone();
    assert_eq!(
        child_key,
        evacuation_key(fed(1), fed(2), child_occurrence),
        "replacement keeps the allocator's deterministic evacuation identity"
    );
    ...
    assert!(matches!(
        ...
        Action::Evacuate {
            ...
            && *to == fed(2)
            && *fresh_amount == Msat(300_000)
            && *components == cap_b
            && *fee_cap == cap_b.at(*fresh_amount)
    ));

    let fresh_balances = BTreeMap::from([(fed(1), Msat(1_000_000)), (fed(2), Msat(0))]);
    // This is deliberately admitted *after* planning.  Its token is fresh, so
    // a refusal below cannot be explained away as a stale-generation guard:
    // commit must rescan durable shadow reservations before it releases the
    // old marker and installs the replacement child.
    let user_key = IdempotencyKey("user:reservation-raced-replacement".to_owned());
    client
        .decide_op(move_request(
            &user_key.0,
            Action::DirectInflow {
        ...
        .await
        .expect("admit independent user reservation after replacement planning");
    tokio::time::timeout(
        std::time::Duration::from_secs(1),
        executor.user_started.notified(),
    )
    .await
    .expect("user reservation remains live through replacement commit");
    let reservation_race_facts = client
        .issue_balance_facts_token()
        .await
        .expect("facts sampled after user reservation admission");
    let reservation_race = client
        .commit_tick(round, fresh_balances.clone(), reservation_race_facts)
        .await
        .expect("current shadow reservation is a scoped refusal");
    assert!(
        reservation_race.accepted.is_empty(),
        "{reservation_race:#?}"
    );
    assert!(
        reservation_race
            .refused
            .iter()
            .any(|refusal| refusal.key == child_key && refusal.reason == RefuseReason::OverCap),
        "{reservation_race:#?}"
    );
    let retained_parent = journal
        .get(&old_key)
        .await
        .expect("read parent after pre-admission refusal")
        .expect("parent survives reservation refusal");
    assert_eq!(retained_parent.status, IntentStatus::Pending);
    assert_eq!(
        retained_parent.evacuation_refusal,
        Some(evidence.clone()),
        "an over-cap pre-admission refusal retains the exact marker without terminalizing it"
    );
    assert!(
        journal
            .get(&child_key)
            ...
            .is_none(),
        "reservation refusal happens before a replacement child is durable"
    );
    executor.release_user.notify_one();
    tokio::time::timeout(std::time::Duration::from_secs(1), async {
        loop {
            if journal
                .get(&user_key)
                .await
                .expect("read independent user work")
                .is_some_and(|intent| intent.status == IntentStatus::Done)
            {
                return;
            }
    ...
    .await
    .expect("independent user work remains live and can finish");

    let reconcile = client
        .reconcile()
        .await
        .expect("retained parent receives planner ownership");
    let round = client
        .decide_tick_round(ProbeFacts {
            probes: vec![(fed(1), dying_probe(300_000)), (fed(2), healthy_probe(0))],
            occurrence: child_occurrence,
        ...
        .await
        .expect("plan replacement again after independent user work finishes");
    assert_eq!(
        round
            .replacement
            .as_ref()
            .expect("retained evidence replans after the scoped refusal")
            .fresh
            .idempotency_key,
        child_key
    );
    // The balance sample is taken only after the second replacement plan and
    // the actor-issued token is captured immediately before its commit.
    let balance_facts = client
        .issue_balance_facts_token()
        .await
        .expect("issue balance facts for fresh balances");
    let generations_before_exchange = balance_facts.generations.clone();
    // The write commits, then the journal reports a transport-shaped error.
    // CommitTick must prove this exact parent/child/sidecar outcome rather
    // than retrying the exchange (which would install a second driver or
    // create a third identity).
    journal.fail_after_next_evacuation_replacement_with_confirmation_read_for_test();
    journal.stop_after_evacuation_replacement_before_child_driver_for_test();
    let report = client
        .commit_tick(round, fresh_balances, balance_facts)
        .await
        .expect("confirm the exact post-commit replacement");
    assert_eq!(report.accepted, vec![child_key.clone()], "{report:#?}");
    assert!(report.refused.is_empty(), "{report:#?}");
    assert_eq!(
        registry_size(&client).await,
        0,
        "test seam stops after the durable exchange, before a child driver/cache can exist"
    );
    let generations_after_exchange = client
        .issue_balance_facts_token()
        .await
        .expect("replacement leaves fresh balance facts issuable")
        .generations;
    for endpoint in [fed(1), fed(2)] {
        assert!(
            generations_after_exchange[&endpoint]
                > *generations_before_exchange.get(&endpoint).unwrap_or(&0),
            "the accepted exchange invalidates its {endpoint:?} balance fact"
        );
    }
    ...
    let old = journal
        ...
        .expect("retired parent exists");
    let child = journal
        .get(&child_key)
        .await
        .expect("read fresh child")
        .expect("fresh child exists");
    assert_eq!(old.status, IntentStatus::Failed);
    assert_eq!(old.idempotency_key, old_key);
    assert_eq!(old.attempt, 0);
    assert_eq!(old.action, old_action);
    assert_eq!(
        old.actor,
        Actor::Agent {
    ...
    assert_eq!(old.evacuation_refusal, Some(evidence));
    assert_eq!(child.status, IntentStatus::Pending);
    ...
    assert_eq!(child.idempotency_key, child_key);
    assert_eq!(child.evacuation_refusal, None);

    let neighbors_from_old = journal
        .evacuation_supersession_neighbors(&old_key)
        .await
        .expect("read forward supersession link");
    assert_eq!(
        neighbors_from_old
            .successor
            .as_ref()
            .map(|link| &link.new_key),
        Some(&child_key)
    );
    assert!(neighbors_from_old.predecessor.is_none());
    let neighbors_from_child = journal
        .evacuation_supersession_neighbors(&child_key)
        .await
        .expect("read backward supersession link");
    assert_eq!(
        neighbors_from_child
            .predecessor
            .as_ref()
            .map(|link| &link.old_key),
        Some(&old_key)
    );
    assert!(neighbors_from_child.successor.is_none());

    // While the executor blocks the child, every live projection has exactly
    // the child: the strict actor snapshot, the allocator projection (there is
    // no MoveRecord yet), and the conflict blocker set.
    let strict = match client
        .snapshot(SnapshotScope::Reservations)
        .await
        .expect("strict reservation snapshot")
    {
        Snapshot::Reservations(reservations) => reservations,
        other => panic!("wrong snapshot: {other:?}"),
    };
    assert_eq!(
        strict.outbound(fed(1)),
        Msat(300_000 + cap_b.at(Msat(300_000)).0)
    );
    assert_eq!(strict.inbound(fed(2)), Msat(300_000));
    let live = journal.pending().await.expect("scan live replacement");
    assert_eq!(live, vec![child.clone()], "only the child remains live");
    let allocator = wallet_core::project_allocator_reservations(&live, &BTreeMap::new());
    assert_eq!(
        allocator, strict,
        "unartifactual child has the strict allocation hold"
    );
    let blockers = wallet_core::GoalBlockers::from_intents(&live);
    let mut same_goal_contender = fresh.clone();
    same_goal_contender.idempotency_key = IdempotencyKey("evac:br-n8o-contender".to_owned());
    assert!(
        blockers.blocks_decision(
            &same_goal_contender,
            Actor::Agent {
                occurrence: child_occurrence
            }
        ),
        "the only live goal blocker is the child"
    );
    assert_eq!(
        executor.calls.load(Ordering::SeqCst),
        2,
        "only the old refusal and independent user work ran before process loss"
    );
    assert_eq!(
        executor
            .performed
            ...
            .collect::<Vec<_>>(),
        vec![old_key.clone(), user_key.clone()],
        "the restart seam leaves the successor durable but un-driven in the old process"
    );

    // Drop the entire old service/actor/driver registry and reopen a NEW
    // journal handle over the same database.  No derived MoveRecord exists
    // for this stopped-before-driver child; if a future implementation leaves
    // one, the test still derives reservations and ownership from the intent,
    // not from process-local state.
    service
        .shutdown()
        .await
        .expect("tear down old service and registry");
    let restarted_journal = Arc::new(journal.reopen_for_test());
    let restart_executor = Arc::new(wallet_core::MockExecutor::new());
    let restarted = WalletService::start_parts(
        None,
    ...
    .await
    .expect("start a new service/runtime over the same database");
    let restarted_client = restarted.client();

    // Before driving, the fresh actor sees exactly the linked child in every
    // live projection; the old terminal parent remains only as audit evidence.
    let restarted_live = restarted_journal
        .pending()
        .await
        .expect("scan durable live work after cache loss");
    assert_eq!(restarted_live, vec![child.clone()]);
    let restarted_reservations =
        wallet_core::project_allocator_reservations(&restarted_live, &BTreeMap::new());
    assert_eq!(restarted_reservations, strict);
    assert!(
        wallet_core::GoalBlockers::from_intents(&restarted_live).blocks_decision(
            &same_goal_contender,
            Actor::Agent {
                occurrence: child_occurrence
            }
    ...
    assert_eq!(registry_size(&restarted_client).await, 0);

    // Reconciliation sees the durable child as the sole live owner.  It must
    // neither re-drive the retired parent nor mistake the post-commit error
    // for permission to attempt another exchange.
    let reconciled = restarted_client
        .reconcile()
        .await
        .expect("reconcile confirmed replacement");
    assert_eq!(reconciled.redriven, 1, "{reconciled:#?}");
    assert_eq!(
        registry_size(&restarted_client).await,
        0,
        "the new mock executor terminalizes its one rehydrated child"
    );
    assert_eq!(
        restarted_journal
            .pending()
            .await
            .expect("scan after restarted reconciliation"),
        Vec::<Intent>::new(),
        "restart replay consumes only the linked child"
    );
    assert_eq!(
        restart_executor.performed_keys(),
        vec![child_key.clone()],
        "a fresh service drives exactly one child identity exactly once"
    );
    let done = restarted_journal
        .get(&child_key)
        .await
        .expect("read child terminal after restart")
        .expect("child remains auditable after restart");
    assert_eq!(done.attempt, 0);
    assert_eq!(done.status, IntentStatus::Done);
    restarted
        .shutdown()
        .await
        .expect("shutdown restarted service");
}

fn qualifying_marker(
    from: FederationId,
    to: FederationId,
    occurrence: Occurrence,
) -> (Intent, EvacuationRefusalEvidence, EvacFeeCap) {
    let old_cap = EvacFeeCap {
        base_msat: Msat(10),
        bps: 0,
    };
    let new_cap = EvacFeeCap {
        base_msat: Msat(20),
        bps: 0,
    };
    let amount = Msat(100);
    let evidence = EvacuationRefusalEvidence {
        ...
        diagnostic: "controlled qualifying structural refusal".to_owned(),
        measured_at_ms: 1,
    };
    let parent = Intent {
        idempotency_key: evacuation_key(from, to, occurrence),
        ...
        max_fee: Some(old_cap.at(amount)),
        status: IntentStatus::Pending,
        reason: ReasonCode::ShutdownNotice,
        actor: Actor::Agent { occurrence },
        created_at_ms: 1,
    ...
    (parent, evidence, new_cap)
}

/// A corrupt strict reservation row fails closed: reconciliation must retain the exact planner
/// marker rather than invoke an executor outside normal admission/replacement authority.
#[tokio::test]
async fn corrupt_reservation_view_keeps_exact_marked_parent_parked_without_driver_or_wake() {
    let db = MemDatabase::new().into_database();
    let journal = Arc::new(FedimintJournal::new(db.clone()));
    let (parent, _old_evidence, new_cap) = qualifying_marker(fed(1), fed(2), Occurrence(730));
    journal.upsert(&parent).await.expect("seed marked parent");
    let app_db = db.with_prefix(vec![0x00]);
    let mut parent_key = vec![0x01];
    parent_key.extend_from_slice(parent.idempotency_key.0.as_bytes());
    let mut parent_tx = app_db.begin_transaction_nc().await;
    let parent_bytes = parent_tx
        .raw_get_bytes(&parent_key)
        .await
        .expect("read seeded parent bytes")
        .expect("seeded parent bytes");

    let poison_key = IdempotencyKey("move:corrupt-fallback-reservation".to_owned());
    let mut intent_key = vec![0x01];
    intent_key.extend_from_slice(poison_key.0.as_bytes());
    let mut pending_index_key = vec![0x04, 0x00];
    pending_index_key.extend_from_slice(poison_key.0.as_bytes());
    ...
    let report = client
        .reconcile()
        .await
        .expect("poison-tolerant reconciliation still recovers durable ownership");
    assert_eq!(
        report.redriven, 0,
        "the marked parent remains planner-owned while strict admission is unreadable: {report:#?}"
    );
    assert!(
        tokio::time::timeout(std::time::Duration::from_millis(25), wake.changed())
            .await
            .is_err(),
        "a corrupt reservation view does not create a scheduler wake"
    );
    assert_eq!(
        executor.calls.load(Ordering::SeqCst),
        0,
        "corruption recovery must not launch an exact-parent executor bypass"
    );
    let mut after_tx = app_db.begin_transaction_nc().await;
    assert_eq!(
        after_tx
            .raw_get_bytes(&parent_key)
            .await
            .expect("read retained parent bytes")
            .expect("retained parent bytes"),
        parent_bytes,
        "fail-closed recovery retains the full marked parent byte-for-byte"
    );
    assert!(
        journal.reservation_intents().await.is_err(),
        "fallback execution does not weaken or repair strict admission"
    );
    service.shutdown().await.expect("shutdown");
}

/// A scheduler-owned marker clear is durable before its retried driver can write a renewed
/// structural refusal. A retryable post-commit error consumes the one-shot suppression, while a
/// permanent pre-commit refusal must leave a later repaired marker wake visible.
#[tokio::test]
async fn marker_clear_suppression_only_covers_retryable_ambiguous_errors() {
    let db = MemDatabase::new().into_database();
    let journal = Arc::new(FedimintJournal::new(db));
    let (parent, evidence, new_cap) = qualifying_marker(fed(1), fed(2), Occurrence(731));
    journal.upsert(&parent).await.expect("seed marked parent");
    let executor = Arc::new(EvidenceStructuralRefusalExecutor {
        calls: AtomicUsize::new(0),
        evidence,
    ...
    let first = client
        .reconcile()
        .await
        .expect("first reconcile parks the exact marker");
    assert_eq!(first.redriven, 0);

    // The clear commits then reports an error, and its exact confirmation read faults. The
    // conservative suppression must still cover the markerless parent the same reconcile drives.
    ...
    let second = client
        .reconcile()
        .await
        .expect("second reconcile releases and re-drives the exact parent");
    assert_eq!(second.redriven, 1);
    tokio::time::timeout(std::time::Duration::from_secs(1), async {
        loop {
            let current = journal
                .get(&parent.idempotency_key)
                .await
                .expect("read re-marked parent")
    ...
    .await
    .expect("postcommit reset leaves the exact renewed marker");
    assert!(
        tokio::time::timeout(std::time::Duration::from_millis(25), wake.changed())
            .await
            .is_err(),
        "the postcommit reread consumes the clear's one-shot wake suppression"
    );
    service.shutdown().await.expect("shutdown");

    // A permanent pre-transaction refusal is not an ambiguous commit boundary. After an
    // operator-equivalent repair replaces this retained marker with a changed markerless snapshot,
    // its retried driver must publish the next qualifying structural marker rather than have the
    // old clear swallow that wake.
    let db = MemDatabase::new().into_database();
    let journal = Arc::new(FedimintJournal::new(db));
    let (parent, evidence, new_cap) = qualifying_marker(fed(3), fed(4), Occurrence(732));
    journal.upsert(&parent).await.expect("seed marked parent");
    let executor = Arc::new(EvidenceStructuralRefusalExecutor {
        calls: AtomicUsize::new(0),
        evidence,
    ...
    journal.fail_before_next_marker_clear_permanently_for_test();
    assert_eq!(
        client
            .reconcile()
            .await
            .expect("a marker-local permanent clear refusal does not halt durable recovery")
            .redriven,
        0,
        "the failed clear leaves its parent marked and unclaimed"
    );
    let mut repaired = parent.clone();
    repaired.evacuation_refusal = None;
    repaired.created_at_ms = repaired.created_at_ms.saturating_add(1);
    journal
        .upsert(&repaired)
        .await
        .expect("repair replaces the definitely uncommitted marker with a changed snapshot");
    assert_eq!(
        client
            .reconcile()
            .await
            .expect("repaired parent regains an ordinary driver")
            .redriven,
        1
    );
    tokio::time::timeout(std::time::Duration::from_secs(1), async {
    ...
    .await
    .expect("repaired parent resets to a qualifying marker");
    tokio::time::timeout(std::time::Duration::from_secs(1), wake.changed())
        .await
        .expect("a permanent clear error must not suppress the later qualifying wake")
        .expect("wake sender remains live");
    service.shutdown().await.expect("shutdown");
}

/// A successful policy update invalidates a prior scheduler generation's clear suppression. Its
/// own wake may already have been consumed before an in-flight driver renews the marker, so that
/// renewed qualifying marker must issue a fresh wake.
#[tokio::test]
async fn put_policy_clears_held_marker_clear_suppression_before_driver_reset() {
    let db = MemDatabase::new().into_database();
    let journal = Arc::new(FedimintJournal::new(db));
    let (parent, evidence, new_cap) = qualifying_marker(fed(1), fed(2), Occurrence(732));
    journal.upsert(&parent).await.expect("seed marked parent");
    let executor = Arc::new(HeldStructuralRefusalExecutor {
        calls: AtomicUsize::new(0),
        evidence,
        started: Notify::new(),
        release: Notify::new(),
    ...
    assert_eq!(
        client
            .reconcile()
            .await
            .expect("first reconcile parks marker")
            .redriven,
        0
    );
    let driver_started = executor.started.notified();
    assert_eq!(
        client
            .reconcile()
            .await
            .expect("second reconcile clears and starts parent")
            .redriven,
        1
    );
    tokio::time::timeout(std::time::Duration::from_secs(1), driver_started)
        .await
        .expect("driver is held after the durable marker clear");

    let policy = client.get_policy().await.expect("read installed policy");
    client
        .put_policy(policy)
        .await
    ...
    .await
    .expect("held driver resets to the qualifying marker");
    tokio::time::timeout(std::time::Duration::from_secs(1), wake.changed())
        .await
        .expect("post-policy structural reset must issue a new wake")
        .expect("wake sender remains live");
    service.shutdown().await.expect("shutdown");
}

async fn qualifying_replacement_round(
    client: &WalletClient,
    ...
    let (parent, evidence, new_cap) = qualifying_marker(fed(1), fed(2), old_occurrence);
    journal
        .upsert(&parent)
        .await
    ...
    client
        .put_policy(policy)
        .await
        .expect("install qualifying cap");
    let round = client
        .decide_tick_round(ProbeFacts {
            probes: vec![(fed(1), dying_probe(100)), (fed(2), healthy_probe(0))],
            occurrence: child_occurrence,
            now_ms: 2,
            price_routes: false,
            blocked: wallet_core::GoalBlockers::default(),
            admission_snapshot: client.issue_tick_plan_token().await.expect("plan token"),
        })
        .await
        .expect("qualifying marker plans a replacement");
    assert!(round.decisions.is_empty(), "{round:#?}");
    assert!(round.replacement.is_some(), "{round:#?}");
    assert!(round.marker_disposition.is_none(), "{round:#?}");
    (
        parent,
        evidence,
        round,
        // The planning probe sizes the dying source at 100, while this fresh
        // commit sample leaves enough room for the increased 20-msat cap.
        BTreeMap::from([(fed(1), Msat(1_000)), (fed(2), Msat(0))]),
    )
}

/// Build a replacement from the production scheduler sequence: reconciliation first parks the
/// exact structural parent, then planning receives that reconciliation's admission snapshot.  The
/// error-path tests below must exercise this shape because an unparked direct `CommitTick` cannot
/// detect a missing `consume_parked_evacuation_marker`.
async fn parked_qualifying_replacement_round(
    client: &WalletClient,
    journal: &FedimintJournal,
    old_occurrence: Occurrence,
    child_occurrence: Occurrence,
    ...
    let reconciliation = client
        .reconcile()
        .await
        .expect("production reconciliation parks the exact marked parent");
    assert_eq!(
        client
            .parked_evacuation_handoff_state_for_test()
            .await
            .expect("inspect parked parent before planning"),
        (1, true),
        "the real reconciliation must offer the parent that a failed commit later consumes"
    );
    let round = client
        .decide_tick_round(ProbeFacts {
            probes: vec![(fed(1), dying_probe(100)), (fed(2), healthy_probe(0))],
            ...
            admission_snapshot: reconciliation.admission_snapshot,
        })
        .await
        .expect("qualifying parked marker plans a replacement");
    assert!(round.decisions.is_empty(), "{round:#?}");
    assert!(round.replacement.is_some(), "{round:#?}");
    assert!(round.marker_disposition.is_none(), "{round:#?}");
    (
...
/// A caller can forge a TickRound even though the planner never emits both outcomes. Reject that
/// shape before authority validation or either marker's durable clear: neither parent authorized
/// the other's disposition.
#[tokio::test]
async fn forged_dual_replacement_and_disposition_round_retains_both_markers() {
    let (service, journal) = fixture(Arc::new(SlowExecutor::default())).await;
    let client = service.client();
    let (replacement_parent, _evidence, mut round, balances) =
        qualifying_replacement_round(&client, &journal, Occurrence(820), Occurrence(821)).await;
    let replacement_child = round
        .replacement
        .as_ref()
        .expect("fixture supplies replacement")
        .fresh
        .idempotency_key
        .clone();
    let (disposition_parent, _disposition_evidence, _) =
        qualifying_marker(fed(3), fed(4), Occurrence(819));
    journal
        .upsert(&disposition_parent)
        .await
        .expect("seed distinct disposition parent");
    round.marker_disposition = Some(EvacuationMarkerDisposition {
    ...
    let error = client
        ...
        .await
        .expect_err("forged dual marker outcomes must be rejected");
    assert!(
        matches!(error, ServiceError::Storage(ref message) if message.contains("cannot share a round")),
        "{error:?}"
    );
    assert_eq!(
        journal
        ...
        Some(replacement_parent),
        "dual-round rejection must not clear the replacement marker"
    );
    assert_eq!(
        journal
            .get(&disposition_parent.idempotency_key)
            .await
            .expect("read retained disposition parent"),
        Some(disposition_parent),
        "dual-round rejection must not clear the disposition marker"
    );
    assert_eq!(
        journal
            .get(&replacement_child)
            .await
            .expect("read replacement child"),
        None,
        "the malformed round must not admit a child"
    );
    service.shutdown().await.expect("shutdown");
}

#[derive(Clone, Copy)]
enum ReplacementPreExchangeFault {
    ParentRead,
    PendingScan,
    ReservationProjection,
}

async fn assert_replacement_pre_exchange_storage_fault_retains_marker_and_terminalizes_tick(
    fault: ReplacementPreExchangeFault,
) {
    let (service, journal) = fixture(Arc::new(SlowExecutor::default())).await;
    let client = service.client();
    let (parent, _evidence, round, balances) =
        parked_qualifying_replacement_round(&client, &journal, Occurrence(830), Occurrence(831))
            .await;
    let child_key = round
        .replacement
        .as_ref()
        ...
        .idempotency_key
        .clone();
    let tick_key = IdempotencyKey(format!(
        "tick:replacement-preexchange-storage:{}",
        match fault {
            ReplacementPreExchangeFault::ParentRead => "parent",
            ReplacementPreExchangeFault::PendingScan => "pending",
            ReplacementPreExchangeFault::ReservationProjection => "reservation",
        }
    ));
    journal
        .record_tick_started(&tick_key, Occurrence(831), 1)
        .await
        .expect("seed Started tick");
    let facts = client
        .issue_balance_facts_token()
        .await
        .expect("fresh facts before one-shot pre-exchange fault");
    match fault {
        ReplacementPreExchangeFault::ParentRead => {
            journal.fail_one_intent_read_after_successes_for_test(0);
        }
        // CommitTick's complete-batch scans happen before the replacement
        // rechecks its own authority. Let that first scan succeed and fault
        // precisely the second, pre-exchange scan.
        ReplacementPreExchangeFault::PendingScan => {
            journal.fail_one_pending_read_after_successes_for_test(1);
        }
        ReplacementPreExchangeFault::ReservationProjection => {
            journal.fail_one_reservation_read_after_successes_for_test(1);
        }
    }
    let error = client
        .commit_tick_with_facts(round, balances, facts, Some(tick_key.clone()))
        .await
        .expect_err("a pre-exchange storage failure remains a truthful storage error");
    assert!(
        matches!(error, ServiceError::Storage(_)),
        "pre-exchange error must not be recast as a refusal: {error:?}"
    );
    let parent_after = journal
        .get(&parent.idempotency_key)
        .await
        .expect("read parent after one-shot fault")
        .expect("parent survives pre-exchange fault");
    assert_eq!(parent_after.status, IntentStatus::Pending);
    assert_eq!(
        parent_after.evacuation_refusal, parent.evacuation_refusal,
        "a pre-exchange storage fault retains the exact structural marker for repair"
    );
    assert_eq!(
        parent_after.action, parent.action,
        "marker clear must not rewrite the uncommitted parent"
    );
    assert_eq!(
        journal
            .get(&child_key)
        ...
        None,
        "no child can exist before the exchange"
    );
    assert_eq!(
        journal
            .replacement_child_namespace(&child_key)
            .await
            .expect("inspect pristine prospective child namespace"),
        crate::journal::ReplacementChildNamespace::Pristine
    );
    let recaptured = client
        .reconcile()
        .await
        .expect("the next healthy reconciliation recaptures the retained marker");
    assert_eq!(
        recaptured.redriven, 0,
        "a retained replacement marker remains planner-owned: {recaptured:#?}"
    );
    assert_eq!(
        journal
            .get(&parent.idempotency_key)
            .await
            .expect("read parent after recapture"),
        Some(parent.clone()),
        "recapturing a pre-exchange failure must not clear its exact marker"
    );
    assert_eq!(
        journal
            .operation(&crate::journal::OperationRef::Key(tick_key))
            .await
            .expect("read tick audit")
            .expect("Started tick remains auditable")
            .status,
        OperationStatus::Failed,
        "a pre-exchange storage return must terminalize the Started scheduler tick"
    );
    service.shutdown().await.expect("shutdown");
}

#[tokio::test]
async fn replacement_parent_read_storage_fault_retains_marker_and_terminalizes_tick() {
    assert_replacement_pre_exchange_storage_fault_retains_marker_and_terminalizes_tick(
        ReplacementPreExchangeFault::ParentRead,
    )
    .await;
}
...
#[tokio::test]
async fn replacement_pending_scan_storage_fault_retains_marker_and_terminalizes_tick() {
    assert_replacement_pre_exchange_storage_fault_retains_marker_and_terminalizes_tick(
        ReplacementPreExchangeFault::PendingScan,
    )
    .await;
...
#[tokio::test]
async fn replacement_reservation_projection_storage_fault_retains_marker_and_terminalizes_tick() {
    assert_replacement_pre_exchange_storage_fault_retains_marker_and_terminalizes_tick(
        ReplacementPreExchangeFault::ReservationProjection,
    )
    .await;
...
#[tokio::test]
async fn replacement_round_audits_deferred_executable_and_advisory_without_admitting_siblings() {
    let executor = Arc::new(SlowExecutor::default());
    let (service, journal) = fixture(executor.clone()).await;
    let client = service.client();
    let (_parent, _evidence, mut round, balances) =
        qualifying_replacement_round(&client, &journal, Occurrence(624), Occurrence(625)).await;
    let executable = one_msat_funding(
        "move:replacement-deferred-executable",
        fed(3),
        fed(4),
        Occurrence(625),
    );
    let advisory = AllocatorDecision {
        ...
        reason: ReasonCode::SpendingBelowTarget,
        occurrence: Occurrence(625),
        idempotency_key: IdempotencyKey("refuse:replacement-deferred-advisory".into()),
    };
    round.replacement_deferred = vec![executable.clone(), advisory.clone()];
    let report = client
        .commit_tick(
            round,
            balances.clone(),
        ...
        .await
        .expect("replacement child commits");
    assert_eq!(report.accepted.len(), 1, "{report:#?}");
    assert!(
        !report.accepted.contains(&executable.idempotency_key)
            && !report.accepted.contains(&advisory.idempotency_key),
        "only the replacement child is admitted"
    );
    let executable_row = journal
        .operation(&crate::journal::OperationRef::Key(IdempotencyKey(format!(
            "tick-drop:{}:{}",
            executable.occurrence.0, executable.idempotency_key.0
        ))))
        .await
        .expect("read executable deferred audit")
        .expect("executable deferred operation row");
    assert!(
        executable_row
            .error
            .as_deref()
    ...
    let advisory_row = journal
        ...
        .expect("advisory deferred operation row");
    assert_eq!(
    ...
    assert_eq!(
        ...
        OperationKind::Refusal {
            fed: fed(5),
            diagnostics: match advisory.action {
                Action::RefuseInflow { diagnostics, .. } => diagnostics,
                _ => unreachable!("advisory fixture"),
            },
        },
        "the advisory's source/want/available/threshold diagnostics must not be defaulted"
    );
    tokio::time::timeout(std::time::Duration::from_secs(1), async {
        while executor.calls.load(Ordering::SeqCst) != 1 {
            tokio::task::yield_now().await;
    ...
    .await
    .expect("only the replacement child executes");
    service.shutdown().await.expect("shutdown");
}

#[tokio::test]
async fn stale_replacement_occurrence_refusal_retains_the_typed_marker() {
    let executor = Arc::new(SlowExecutor::default());
    let (service, journal) = fixture(executor).await;
    let client = service.client();
    let old_occurrence = Occurrence(u64::MAX - 1);
    let (parent, _evidence, round, balances) =
        qualifying_replacement_round(&client, &journal, old_occurrence, old_occurrence).await;
    ...
    let report = client
        ...
        .await
        .expect("stale occurrence is a refusal, not an actor fault");
    assert!(
        matches!(
            ...
            [TickRefusal {
                ...
                message,
                ..
            }] if message.contains("requires --occurrence advanced beyond old Agent occurrence")
        ),
        "{report:#?}"
    );
    assert_eq!(
        ...
        Some(parent.clone()),
        "a near-max pre-exchange stale occurrence is operator-correctable and must retain its typed marker"
    );
    let reconcile = client.reconcile().await.expect(
        "scheduler reconciliation parks the retained near-max parent for its replacement plan",
    );
    assert_eq!(
        reconcile.redriven, 0,
        "a parent below MAX remains planner-owned until its strictly newer child commits"
    );
    let successor = client
        ...
        .await
        .expect("near-max parent can plan the one strictly newer MAX child");
    let child = successor
        .replacement
        .as_ref()
        .expect("strictly newer replacement")
        .fresh
        .clone();
    assert_eq!(child.occurrence, Occurrence(u64::MAX));
    let committed = client
        .commit_tick(
            ...
            client
                .issue_balance_facts_token()
                .await
                .expect("fresh successor facts"),
        )
        .await
        .expect("MAX child is valid only as the strictly newer handoff");
    assert_eq!(committed.accepted, vec![child.idempotency_key.clone()]);
    assert_eq!(
        journal
            .get(&parent.idempotency_key)
            .await
            .expect("read retired near-max parent")
...
/// The production cycle is `ReconcileDecide` -> plan -> `CommitTick`, so the reconciliation that
/// hands a parent to the replacement planner also parks its exact snapshot. A stale-occurrence
/// refusal deliberately retains the durable marker for a later, strictly newer child — which only
/// holds if it consumes that parked snapshot too. Otherwise the next `ReconcileDecide` drains the
/// snapshot and full-parent-CAS releases the very evidence the refusal preserved.
#[tokio::test]
async fn stale_replacement_refusal_consumes_its_parked_snapshot_for_the_next_reconcile() {
    let executor = Arc::new(SlowExecutor::default());
    let (service, journal) = fixture(executor.clone()).await;
    let client = service.client();
    let (parent, evidence, new_cap) = qualifying_marker(fed(1), fed(2), Occurrence(700));
    journal
        .upsert(&parent)
        .await
        .expect("seed qualifying marker");
    ...
    client
        ...
        .await
        .expect("install qualifying cap");
    let balances = BTreeMap::from([(fed(1), Msat(1_000)), (fed(2), Msat(0))]);

    let first = client
        .reconcile()
        .await
        .expect("the scheduler cycle parks the parent it hands to the planner");
    assert_eq!(
        first.redriven, 0,
        "a captured planner marker starts no recovery driver: {first:#?}"
    );
    let stale = client
        .decide_tick_round(ProbeFacts {
            probes: vec![(fed(1), dying_probe(100)), (fed(2), healthy_probe(0))],
            occurrence: Occurrence(700),
            now_ms: 2,
            price_routes: false,
            blocked: first.blocked,
            admission_snapshot: first.admission_snapshot,
        })
        .await
        .expect("the qualifying marker plans a replacement");
    assert!(stale.replacement.is_some(), "{stale:#?}");
    let report = client
        .commit_tick(
            stale,
            balances.clone(),
    ...
    assert_eq!(
        journal
            ...
            .expect("read marked parent"),
        Some(parent.clone()),
        "the pre-exchange stale refusal retains its exact typed marker"
    );

    let second = client
        .reconcile()
        .await
        .expect("the next scheduler cycle recaptures the retained marker");
    assert_eq!(
        second.redriven, 0,
    ...
    assert_eq!(
        journal
            ...
            .evacuation_refusal,
        Some(evidence),
        "the stale refusal's retained evidence must survive the following reconciliation"
    );

    let successor = client
        .decide_tick_round(ProbeFacts {
            probes: vec![(fed(1), dying_probe(100)), (fed(2), healthy_probe(0))],
            occurrence: Occurrence(701),
            now_ms: 3,
            price_routes: false,
            blocked: second.blocked,
            admission_snapshot: second.admission_snapshot,
        })
        .await
        .expect("the retained marker plans its strictly newer child");
    let child = successor
        .replacement
        .as_ref()
        .expect("strictly newer replacement")
        .fresh
        .clone();
    assert_eq!(child.occurrence, Occurrence(701));
    let committed = client
        .commit_tick(
            successor,
            balances,
        ...
        .await
        .expect("the strictly newer child commits");
    assert_eq!(committed.accepted, vec![child.idempotency_key.clone()]);
    assert_eq!(
        journal
            .get(&parent.idempotency_key)
...
/// A stale prospective-child namespace is rejected inside the exchange
/// autocommit closure, before that transaction can commit. That definite
/// validation failure retains the parent marker and must not poison the fresh
/// scheduler capabilities needed to choose the next occurrence.
#[tokio::test]
async fn precommit_stale_child_namespace_retains_parked_marker_without_poisoning_scheduler() {
    let executor = Arc::new(SlowExecutor::default());
    let (service, journal) = fixture(executor.clone()).await;
    let client = service.client();
    let (parent, evidence, new_cap) = qualifying_marker(fed(1), fed(2), Occurrence(840));
    journal
        .upsert(&parent)
        .await
        .expect("seed qualifying marked parent");
    let mut policy = client.get_policy().await.expect("read policy");
    policy.per_fed_cap = Msat(1_000);
    policy.spending_target = Msat(100);
    policy.standby_target = Msat(100);
    ...
    let first_reconcile = client
        .reconcile()
        .await
        .expect("real scheduler reconciliation parks the qualifying parent");
    assert_eq!(first_reconcile.redriven, 0, "{first_reconcile:#?}");
    let first_round = client
        .decide_tick_round(ProbeFacts {
            probes: vec![(fed(1), dying_probe(100)), (fed(2), healthy_probe(0))],
            occurrence: Occurrence(841),
            now_ms: 2,
            price_routes: false,
            blocked: first_reconcile.blocked,
            admission_snapshot: first_reconcile.admission_snapshot,
        })
        .await
        .expect("parked parent plans occurrence N");
    let first_child_key = first_round
        .replacement
        .as_ref()
        .expect("qualifying replacement at N")
        .fresh
        .idempotency_key
        .clone();
    journal
        .seed_stale_replacement_child_namespace_for_test(&first_child_key)
        .await
        .expect("seed stale child namespace under occurrence N's key");
    let tick_key = IdempotencyKey("tick:replacement-precommit-stale-child-namespace".to_owned());
    journal
        .record_tick_started(&tick_key, Occurrence(841), 1)
        .await
        .expect("seed Started scheduler tick");
    let error = client
        .commit_tick_with_facts(
            first_round,
            balances.clone(),
            client
                .issue_balance_facts_token()
                .await
                .expect("fresh facts for N"),
            Some(tick_key.clone()),
        )
        .await
        .expect_err("stale child namespace is reported as the journal Storage diagnostic");
    assert!(
        matches!(error, ServiceError::Storage(ref message) if message.contains("child namespace is not empty")),
        "{error:?}"
    );
    assert_eq!(
        journal
            .operation(&OperationRef::Key(tick_key))
            .await
            .expect("read terminal tick")
            .expect("tick audit remains durable")
            .status,
        OperationStatus::Failed,
        "the Started scheduler tick must terminalize"
    );
    assert_eq!(
        journal
            .get(&parent.idempotency_key)
            ...
            .expect("read retained parent"),
        Some(parent.clone()),
        "the exact Pending parent and marker remain durable"
    );
    assert_eq!(
        journal
            .get(&first_child_key)
            .await
            .expect("read occurrence N child"),
        None,
        "the stale namespace is not an admitted child intent"
    );
    assert_eq!(
        registry_size(&client).await,
        0,
        "a pre-commit rejection starts no child driver"
    );
    assert_eq!(
        executor.calls.load(Ordering::SeqCst),
        0,
        "the parked parent remains planner-owned and no child was driven"
    );
    assert!(
        client.issue_tick_plan_token().await.is_ok(),
        "a pre-commit validation rejection must not poison goal admission"
    );
    assert!(
        client.issue_balance_facts_token().await.is_ok(),
        "a pre-commit validation rejection must not poison balance facts"
    );

    let second_reconcile = client
        .reconcile()
        .await
        .expect("the retained marker is recaptured, not drain-cleared");
    assert_eq!(second_reconcile.redriven, 0, "{second_reconcile:#?}");
    assert_eq!(
        journal
            .get(&parent.idempotency_key)
            .await
            .expect("read parent after recapture"),
        Some(parent.clone()),
        "consuming the exact parked handoff prevents the next reconcile from clearing the marker"
    );
    let second_round = client
        .decide_tick_round(ProbeFacts {
            probes: vec![(fed(1), dying_probe(100)), (fed(2), healthy_probe(0))],
            occurrence: Occurrence(842),
            now_ms: 3,
            price_routes: false,
            blocked: second_reconcile.blocked,
            admission_snapshot: second_reconcile.admission_snapshot,
        })
        .await
        .expect("retained parent plans occurrence N+1");
    let second_child = second_round
        .replacement
        .as_ref()
        .expect("qualifying replacement at N+1")
        .fresh
        .clone();
    assert_ne!(
        second_child.idempotency_key, first_child_key,
        "occurrence N+1 isolates the stale namespace under N's child key"
    );
    let report = client
        .commit_tick(
            second_round,
            ...
            client
                .issue_balance_facts_token()
                .await
                .expect("fresh facts for N+1"),
        )
        .await
        .expect("the clean N+1 namespace exchanges and owns its child");
    assert_eq!(report.accepted, vec![second_child.idempotency_key.clone()]);
    wait_for_registry(&client, 1).await;
    let relation = journal
        .evacuation_supersession(&parent.idempotency_key)
        .await
        .expect("read parent-child supersession link")
        .expect("successful N+1 exchange writes the canonical link");
    assert_eq!(relation.old_key, parent.idempotency_key);
    assert_eq!(relation.new_key, second_child.idempotency_key);
    assert_eq!(relation.occurrence, Occurrence(842));
    let retired_parent = journal
        .get(&parent.idempotency_key)
        .await
        .expect("read retired parent")
        .expect("parent remains auditable");
    assert_eq!(retired_parent.status, IntentStatus::Failed);
    assert_eq!(retired_parent.evacuation_refusal, Some(evidence));
    let committed_child = journal
        .get(&second_child.idempotency_key)
        .await
        .expect("read owned child")
        .expect("N+1 child exists");
    assert_eq!(committed_child.action, second_child.action);
    assert!(matches!(
        committed_child.actor,
        Actor::Agent {
            occurrence: Occurrence(842)
        }
    ));
    assert!(
        client.issue_tick_plan_token().await.is_ok(),
        "the successful recovery confirms goal admission remained unpoisoned"
    );
    assert!(
        client.issue_balance_facts_token().await.is_ok(),
        "the successful recovery confirms balance facts remained unpoisoned"
    );
    service.shutdown().await.expect("shutdown");
}

/// An aborted scheduler cycle must discard every in-memory capture while leaving the durable parent
/// untouched. Retaining the vector after dropping only its handoff would make an old snapshot
/// available to a later reconciliation as if it belonged to that later cycle.
#[tokio::test]
async fn abandon_parked_handoff_discards_the_entire_in_memory_marker_queue() {
    let (service, journal) = fixture(Arc::new(SlowExecutor::default())).await;
    let client = service.client();
    let (parent, evidence, cap) = qualifying_marker(fed(1), fed(2), Occurrence(844));
    journal
        .upsert(&parent)
        .await
        .expect("seed qualifying marked parent");
    ...
    client
        .reconcile()
        .await
        .expect("scheduler reconciliation captures the marker");
    assert_eq!(
        client
            .parked_evacuation_handoff_state_for_test()
            .await
            .expect("inspect captured in-memory state"),
        (1, true),
        "the scheduler has one offered parked snapshot before abandoning its cycle"
    );
    client
        .abandon_parked_evacuation_handoff()
        .await
        .expect("abandon in-memory scheduler offer");
    assert_eq!(
        client
            .parked_evacuation_handoff_state_for_test()
            .await
            .expect("inspect abandoned in-memory state"),
        (0, false),
        "abandon must drop the vector as well as the current handoff"
    );
    assert_eq!(
        journal
            .get(&parent.idempotency_key)
            .await
            .expect("read durable marker after in-memory abandon"),
        Some(parent.clone()),
        "in-memory abandon must not issue a durable marker CAS"
    );
    assert_eq!(parent.evacuation_refusal, Some(evidence));
    service.shutdown().await.expect("shutdown");
}

/// Reconcile captures planner-owned markers before minting the admission token. If minting fails,
/// those in-memory captures cannot survive to the next reconcile: the stale handoff would otherwise
/// exact-CAS clear the durable marker before it can be planned again.
#[tokio::test]
async fn reconcile_token_failure_abandons_parked_markers_for_healthy_recovery() {
    let (service, journal) = fixture(Arc::new(SlowExecutor::default())).await;
    let client = service.client();
    let (parent, evidence, cap) = qualifying_marker(fed(1), fed(2), Occurrence(845));
    journal
        .upsert(&parent)
        .await
        .expect("seed qualifying marked parent");
    ...
    let lease = client
        ...
        .await
        .expect("block only admission-token issuance");
    assert!(
        matches!(
        ...
        "reconciliation must fail after capturing when the token cannot be minted"
    );
    assert_eq!(
        client
            .parked_evacuation_handoff_state_for_test()
            .await
            .expect("inspect failed token path's in-memory state"),
        (0, false),
        "token failure must abandon both the parked marker vector and its handoff"
    );
    client
        .end_external_terminal_mutation(lease)
        .await
        .expect("restore healthy admission authority");
    let healthy = client
        .reconcile()
        .await
        .expect("healthy reconciliation recaptures, rather than clears, the marker");
    assert_eq!(
        healthy.redriven, 0,
        "the recaptured marker remains planner-owned: {healthy:#?}"
    );
    let retained = journal
        .get(&parent.idempotency_key)
        .await
        .expect("read parent after healthy reconcile")
        .expect("parent remains durable");
    assert_eq!(retained.status, IntentStatus::Pending);
    assert_eq!(
        retained.evacuation_refusal,
        Some(evidence),
        "the failed token path must abandon in-memory state without a durable clear"
    );
    service.shutdown().await.expect("shutdown");
}

/// A reconciliation can observe several qualifying parents, but one replacement round may consume
/// only one child. The next reconciliation releases only the parent actually handed to the prior
/// round; every unselected full-parent snapshot remains queued with its durable evidence intact.
#[tokio::test]
async fn unselected_parked_marker_survives_for_the_next_one_child_round() {
    let executor = Arc::new(SlowExecutor::default());
    let (service, journal) = fixture(executor).await;
    let client = service.client();
    let (first_parent, _first_evidence, new_cap) =
        qualifying_marker(fed(1), fed(2), Occurrence(720));
    let (second_parent, second_evidence, _) = qualifying_marker(fed(3), fed(4), Occurrence(721));
    journal
        .upsert(&first_parent)
        .await
        .expect("seed first qualifying marker");
    journal
        .upsert(&second_parent)
        .await
        .expect("seed second qualifying marker");
    let mut policy = client.get_policy().await.expect("read policy");
    policy.per_fed_cap = Msat(10_000);
    policy.spending_target = Msat(100);
    policy.standby_target = Msat(100);
    policy.evac_fee_base_msat = new_cap.base_msat;
    policy.evac_fee_bps = new_cap.bps;
    client
        ...
        .expect("install qualifying cap");
    let probes = vec![
        (fed(1), dying_probe(100)),
        (fed(2), healthy_probe(0)),
        (fed(3), dying_probe(100)),
        (fed(4), healthy_probe(0)),
    ];
    let balances = BTreeMap::from([
        (fed(1), Msat(1_000)),
    ...
    let first_reconcile = client
        .reconcile()
        .await
        .expect("capture both qualifying parents");
    assert_eq!(first_reconcile.redriven, 0, "{first_reconcile:#?}");
    let first_round = client
        .decide_tick_round(ProbeFacts {
            probes: probes.clone(),
            occurrence: Occurrence(722),
            now_ms: 2,
            price_routes: false,
            blocked: first_reconcile.blocked,
            admission_snapshot: first_reconcile.admission_snapshot,
        ...
        .await
        .expect("first parked parent plans");
    assert_eq!(
        first_round
            .replacement
            .as_ref()
        ...
        Some(&first_parent),
        "pending-index order selects the first queued parent"
    );
    let first_commit = client
        .commit_tick(
            ...
            client
                .issue_balance_facts_token()
                .await
                .expect("fresh first-round balance facts"),
        )
        .await
        .expect("first replacement commits");
    assert_eq!(first_commit.accepted.len(), 1, "{first_commit:#?}");

    let second_reconcile = client
        .reconcile()
        .await
        .expect("advance to the unselected parked parent");
    assert_eq!(second_reconcile.redriven, 0, "{second_reconcile:#?}");
    assert_eq!(
        journal
            .get(&second_parent.idempotency_key)
            .await
            .expect("read unselected parent")
            .expect("unselected parent remains live")
            .evacuation_refusal,
        Some(second_evidence),
        "the first round must not release an unselected parent's evidence"
    );
    let second_round = client
        .decide_tick_round(ProbeFacts {
            probes,
            occurrence: Occurrence(723),
            now_ms: 3,
            price_routes: false,
            blocked: second_reconcile.blocked,
            admission_snapshot: second_reconcile.admission_snapshot,
        ...
        .await
        .expect("second parked parent plans");
    assert_eq!(
        second_round
            .replacement
            .as_ref()
            .map(|replacement| &replacement.parent),
        Some(&second_parent),
        "the next one-child round must consume the queued successor"
    );
    let second_commit = client
        .commit_tick(
            ...
            client
                .issue_balance_facts_token()
                .await
                .expect("fresh second-round balance facts"),
        )
        .await
        .expect("second replacement commits");
    assert_eq!(second_commit.accepted.len(), 1, "{second_commit:#?}");
    service.shutdown().await.expect("shutdown");
}

/// A clear fault is local to the handed-off parent. Retaining it at the head would make every
/// later marker unreachable, so the actor rotates the exact snapshot and offers the next parent.
#[tokio::test]
async fn failed_parked_marker_clear_rotates_to_an_independent_parent() {
    let executor = Arc::new(SlowExecutor::default());
    let (service, journal) = fixture(executor).await;
    let client = service.client();
    let (blocked_parent, blocked_evidence, new_cap) =
        qualifying_marker(fed(1), fed(2), Occurrence(740));
    let (next_parent, _next_evidence, _) = qualifying_marker(fed(3), fed(4), Occurrence(741));
    journal
        .upsert(&blocked_parent)
        .await
        .expect("seed first qualifying marker");
    journal
        .upsert(&next_parent)
        .await
        .expect("seed independent qualifying marker");
    let mut policy = client.get_policy().await.expect("read policy");
    policy.per_fed_cap = Msat(10_000);
    policy.spending_target = Msat(100);
    policy.standby_target = Msat(100);
    ...
    let reconcile = client
        .reconcile()
        .await
        .expect("a marker-local clear fault does not abort the scheduler cycle");
    assert_eq!(reconcile.redriven, 0, "{reconcile:#?}");
    assert_eq!(
        journal
            .get(&blocked_parent.idempotency_key)
            .await
            .expect("read failed-clear parent")
            .expect("failed-clear parent remains live")
            .evacuation_refusal,
        Some(blocked_evidence),
        "the failed clear stays fail-closed"
    );
    ...
    let round = client
        ...
        .await
        .expect("independent queued parent plans after the clear fault");
    assert_eq!(
        round
            .replacement
            .as_ref()
            .map(|replacement| &replacement.parent),
        Some(&next_parent),
        "the failed-clear parent rotates behind independent work"
    );
    let report = client
        .commit_tick(
            round,
        ...
        .await
        .expect("independent replacement commits");
    assert_eq!(report.accepted.len(), 1, "{report:#?}");
    service.shutdown().await.expect("shutdown");
}

/// A permanently unclearable parked snapshot is local corruption evidence, not permission to
/// skip the strict durable recovery scan for every other intent in the wallet. The snapshot stays
/// parked and its marker stays durable while unrelated live work regains ownership.
#[tokio::test]
async fn parked_marker_release_failure_does_not_suppress_unrelated_durable_recovery() {
    let executor = Arc::new(SlowExecutor::default());
    let (service, journal) = fixture(executor.clone()).await;
    let client = service.client();
    let (parent, evidence, new_cap) = qualifying_marker(fed(1), fed(2), Occurrence(710));
    journal
        .upsert(&parent)
        .await
        .expect("seed qualifying marker");
    ...
    let first = client
        .reconcile()
        .await
        .expect("capture the planner-owned parent snapshot");
    assert_eq!(first.redriven, 0, "the marker is parked: {first:#?}");

    // A second live Agent evacuation holder from the same source makes the exact marker clear a
    // permanent journal error. The independent C -> D intent models unrelated crash-recovery work
    // which must still regain its driver during this reconcile.
    let conflicting = agent_evacuation_intent(fed(1), fed(5), Msat(1), Occurrence(711));
    let unrelated = agent_funding_intent(fed(3), fed(4), Msat(1), Occurrence(712));
    journal
        .upsert(&conflicting)
        .await
        .expect("seed conflicting live evacuation holder");
    journal
        .upsert(&unrelated)
        .await
        .expect("seed unrelated durable work");

    let second = client
        .reconcile()
        .await
        .expect("a marker-local permanent clear error does not abort durable recovery");
    assert_eq!(
        second.redriven, 2,
        "both non-marker intents regain drivers despite the parked-parent clear error: {second:#?}"
    );
    tokio::time::timeout(std::time::Duration::from_secs(1), async {
        while executor.calls.load(Ordering::SeqCst) != 2 {
            tokio::task::yield_now().await;
    ...
    .await
    .expect("unrelated durable work is rehydrated");
    assert_eq!(
        journal
            .get(&parent.idempotency_key)
            .await
            ...
            .evacuation_refusal,
        Some(evidence),
        "the failed exact clear remains fail-closed"
    );
    service.shutdown().await.expect("shutdown");
}

#[tokio::test]
async fn exhausted_agent_marker_is_redriven_not_planner_owned_or_parked() {
    let executor = Arc::new(SlowExecutor::default());
    let (service, journal) = fixture(executor.clone()).await;
    let client = service.client();
    let (parent, _evidence, qualifying_cap) =
        qualifying_marker(fed(1), fed(2), Occurrence(u64::MAX));
    journal
        .upsert(&parent)
        .await
        .expect("seed direct/corrupt exhausted parent");
    let mut policy = client.get_policy().await.expect("read policy");
    policy.evac_fee_base_msat = qualifying_cap.base_msat;
    policy.evac_fee_bps = qualifying_cap.bps;
    client
        .put_policy(policy)
        .await
        .expect("install otherwise-qualifying cap");

    let reconcile = client
        .reconcile()
        .await
        .expect("scheduler reconcile must treat MAX parent as ordinary retryable work");
    assert_eq!(
        reconcile.redriven, 1,
        "an exhausted parent cannot be reserved for an impossible successor"
    );
    tokio::time::timeout(std::time::Duration::from_secs(1), async {
        while executor.calls.load(Ordering::SeqCst) != 1 {
            tokio::task::yield_now().await;
    ...
    .await
    .expect("ordinary redrive owns the exhausted parent");
    let round = client
        .decide_tick_round(ProbeFacts {
            probes: vec![(fed(1), dying_probe(100)), (fed(2), healthy_probe(0))],
            occurrence: Occurrence(u64::MAX),
        ...
        .await
        .expect("ordinary planner remains available");
    assert!(
        round.replacement.is_none() && round.marker_disposition.is_none(),
        "MAX parent must neither claim replacement ownership nor enter the parked-marker clear path: {round:#?}"
    );
    assert_eq!(
        journal
            .get(&parent.idempotency_key)
            .await
            .expect("read exhausted parent")
            .expect("ordinary redrive claimed parent")
            .status,
        IntentStatus::Executing,
        "the only permitted MAX transition is ordinary driver claim, never the planner's parked-marker clear"
    );
    service.shutdown().await.expect("shutdown");
}

/// A qualifying marker first receives its exclusive shadow.  If that shadow no
/// longer emits an evacuation from the marked source, the same round must
/// resume ordinary unrelated planning while carrying an exact clear-only
/// disposition for the marker.
#[tokio::test]
async fn shadow_without_same_source_evacuation_clears_only_its_marker_and_admits_ordinary_work() {
    let executor = Arc::new(SlowExecutor::default());
    let (service, journal) = fixture(executor.clone()).await;
    let client = service.client();
    let (parent, evidence, new_cap) = qualifying_marker(fed(1), fed(2), Occurrence(70));
    journal
        .upsert(&parent)
        .await
        .expect("seed qualifying marker");
    ...
    policy.spending_target = Msat(1_000_000);
    policy.standby_target = Msat(1_000_000);
    policy.spending_fed = Some(fed(3));
    policy.standby_fed = Some(fed(4));
    policy.evac_fee_base_msat = new_cap.base_msat;
    policy.evac_fee_bps = new_cap.bps;
    client
        .put_policy(policy)
        .await
        .expect("install qualifying policy");
    let reconcile = client.reconcile().await.expect("reconcile marker");
    assert_eq!(reconcile.redriven, 0, "marked parent has no old driver");
    let mut wake = service.policy_wake.clone();
    wake.borrow_and_update();
    let round = client
        .decide_tick_round(ProbeFacts {
            // A is healthy, so the shadow has no A-source evacuation.  C -> D
            // is independently ordinary work which the fallback must retain.
            probes: vec![
                (fed(1), healthy_probe(0)),
                (fed(2), healthy_probe(0)),
                (fed(3), healthy_probe(2_000_000)),
                (fed(4), healthy_probe(0)),
            ],
            occurrence: Occurrence(71),
            now_ms: 2,
        ...
        .await
        .expect("shadow falls back to ordinary planning");
    assert!(round.replacement.is_none(), "{round:#?}");
    assert!(round.marker_disposition.is_some(), "{round:#?}");
    let ordinary = funding_key(fed(3), fed(4), Occurrence(71));
    assert!(
        round
            .decisions
            .iter()
            .any(|decision| decision.idempotency_key == ordinary),
        "ordinary fallback retains independent work: {round:#?}"
    );
    assert!(
        !round.decisions.iter().any(|decision| {
            matches!(decision.action, Action::Evacuate { from, .. } if from == fed(1))
        }),
        "the shadow produced no same-source evacuation: {round:#?}"
    );
    let balances = BTreeMap::from([
        (fed(1), Msat(0)),
        (fed(2), Msat(0)),
        (fed(3), Msat(2_000_000)),
        (fed(4), Msat(0)),
    ]);
    let facts = client
        .issue_balance_facts_token()
        .await
        .expect("fresh facts before commit");
    let report = client
        .commit_tick(round, balances, facts)
        .await
        .expect("ordinary fallback commits");
    assert_eq!(report.accepted, vec![ordinary.clone()], "{report:#?}");
    let cleared = journal
        .get(&parent.idempotency_key)
        .await
        .expect("read cleared parent")
        .expect("parent survives");
    assert_eq!(cleared.status, IntentStatus::Pending);
    assert_eq!(
        cleared.evacuation_refusal, None,
        "only the exact marker clears"
    );
    assert_eq!(cleared.action, parent.action);
    assert_eq!(
        executor.calls.load(Ordering::SeqCst),
        1,
        "only unrelated ordinary work gets an immediate driver"
    );
    assert!(
        tokio::time::timeout(std::time::Duration::from_millis(25), wake.changed())
            .await
            .is_err(),
        "marker disposition neither wakes the old driver nor a policy cycle"
    );
    let later = client.reconcile().await.expect("later normal reconcile");
    assert_eq!(
        later.redriven, 1,
        "ordinary pending parent can retry next cycle"
    );
    tokio::time::timeout(std::time::Duration::from_secs(1), async {
        while executor.calls.load(Ordering::SeqCst) != 2 {
            tokio::task::yield_now().await;
    ...
    .await
    .expect("later reconcile starts the old ordinary pending intent");
    assert_eq!(
        evidence,
        parent.evacuation_refusal.expect("fixture evidence")
    );
    service.shutdown().await.expect("shutdown");
}

async fn assert_marker_disposition_clear_fault_keeps_independent_commit(
    permanent: bool,
    post_commit: bool,
) {
    let (service, journal) = fixture(Arc::new(SlowExecutor::default())).await;
    let client = service.client();
    let (parent, evidence, _cap) = qualifying_marker(fed(1), fed(2), Occurrence(730));
    journal
        .upsert(&parent)
        .await
        .expect("seed planner-owned marker");
    let occurrence = Occurrence(731);
    let independent = one_msat_funding(
        "move:marker-clear-fault-independent",
        fed(3),
        fed(4),
        occurrence,
    );
    ...
    let facts = client
        ...
        .await
        .expect("fresh balance facts");
    let mut round = TickRound::for_test(vec![independent.clone()], 0, token);
    // This is the ordinary fallback shape produced by `plan_tick_round_for_marker`: the marker's
    // original blocker/reservation remains in the ordinary plan, while its disposition is clear-only.
    round.marker_disposition = Some(EvacuationMarkerDisposition {
        parent: parent.clone(),
    });
    if post_commit {
    ...
    let report = client
        ...
        .await
        .expect("a marker-local clear fault must not abort independently replanned work");
    assert_eq!(
        report.accepted,
        vec![independent.idempotency_key.clone()],
        "{report:#?}"
    );
    assert!(
        report.refused.is_empty(),
        "the tick reports the independent admission truthfully: {report:#?}"
    );
    assert!(
        journal
            .get(&independent.idempotency_key)
            .await
            .expect("read independent intent")
            .is_some(),
        "the independent decision was durably admitted"
    );
    let parent_after = journal
        .get(&parent.idempotency_key)
        .await
    ...
    if post_commit {
        assert_eq!(
            ...
            "a confirmed post-commit error may already have cleared the exact marker"
        );
    } else {
        assert_eq!(
            parent_after.evacuation_refusal,
            Some(evidence),
            "a pre-commit clear fault leaves durable recovery evidence"
        );
    }
    service.shutdown().await.expect("shutdown");
}
...
#[tokio::test]
async fn retryable_marker_disposition_clear_fault_keeps_independent_commit() {
    assert_marker_disposition_clear_fault_keeps_independent_commit(false, false).await;
}

#[tokio::test]
async fn permanent_marker_disposition_clear_fault_keeps_independent_commit() {
    assert_marker_disposition_clear_fault_keeps_independent_commit(true, false).await;
}

#[tokio::test]
async fn confirmed_postcommit_marker_disposition_clear_error_keeps_independent_commit() {
    assert_marker_disposition_clear_fault_keeps_independent_commit(false, true).await;
}

/// The replacement-parent scan is authoritative even when the marker appears after off-actor
/// planning has begun. If its exclusive shadow produces no same-source evacuation, the returned
/// marker disposition — not an earlier observation — must trigger ordinary fallback planning.
#[tokio::test]
async fn newly_observed_marker_shadow_falls_back_to_unrelated_ordinary_work() {
    let (service, journal) = fixture(Arc::new(SlowExecutor::default())).await;
    let client = service.client();
    let (parent, _evidence, new_cap) = qualifying_marker(fed(1), fed(2), Occurrence(72));
    let mut policy = client.get_policy().await.expect("read policy");
    policy.per_fed_cap = Msat(10_000_000);
    policy.spending_target = Msat(1_000_000);
    policy.standby_target = Msat(1_000_000);
    ...
    client
        .put_policy(policy)
        .await
        .expect("install qualifying policy before the marker exists");
    let admission_snapshot = client.issue_tick_plan_token().await.expect("plan token");
    let pause = journal.pause_before_next_replacement_scan_for_test();
    let planning_client = client.clone();
    ...
    journal
        .upsert(&parent)
        .await
        .expect("publish the qualifying marker after planning starts");
    pause.release();

    let round = planning
        .await
        .expect("planning task does not panic")
        .expect("the newly observed marker falls back to ordinary planning");
    assert!(round.replacement.is_none(), "{round:#?}");
    assert_eq!(
        round
            .marker_disposition
            .as_ref()
            .map(|disposition| &disposition.parent),
        Some(&parent),
        "the authoritative inner scan observed the newly published marker"
    );
    let ordinary = funding_key(fed(3), fed(4), Occurrence(73));
    assert!(
        round
            .decisions
            .iter()
            .any(|decision| decision.idempotency_key == ordinary),
        "the marker's childless shadow must not discard unrelated eligible work: {round:#?}"
    );
    service.shutdown().await.expect("shutdown");
}

#[tokio::test]
async fn confirmed_uncommitted_exchange_error_retains_marker_for_a_higher_occurrence() {
    let (service, journal) = fixture(Arc::new(SlowExecutor::default())).await;
    let client = service.client();
    let (parent, evidence, round, balances) =
        parked_qualifying_replacement_round(&client, &journal, Occurrence(80), Occurrence(81))
            .await;
    let child_key = round
        .replacement
        .as_ref()
    ...
    let facts = client
        .issue_balance_facts_token()
        .await
        .expect("fresh facts before injected error");
    journal.fail_before_next_evacuation_replacement_for_test();
    let report = client
        .commit_tick(round, balances, facts)
        .await
        .expect("exact uncommitted confirmation remains a truthful refusal");
    assert!(report.accepted.is_empty(), "{report:#?}");
    assert_eq!(report.refused.len(), 1, "{report:#?}");
    assert_eq!(report.refused[0].key, child_key.clone());
    assert_eq!(report.refused[0].reason, RefuseReason::Conflict);
    let old = journal
        .get(&parent.idempotency_key)
        .await
        .expect("read parent")
        .expect("parent survives exact uncommitted confirmation");
    assert_eq!(old.status, IntentStatus::Pending);
    assert_eq!(
        old.evacuation_refusal,
        Some(evidence.clone()),
        "a confirmed-uncommitted exchange retains its exact marker"
    );
    assert_eq!(old.action, parent.action);
    assert_eq!(old.attempt, parent.attempt);
    assert_eq!(
        journal.get(&child_key).await.expect("read child"),
        None,
        "uncommitted confirmation proves no child"
    );
    assert_eq!(
        registry_size(&client).await,
        0,
        "no child driver was installed"
    );
    assert_eq!(
        journal
            .replacement_child_namespace(&child_key)
            .await
            .expect("inspect child namespace"),
        crate::journal::ReplacementChildNamespace::Pristine
    );
    assert_eq!(
        evidence.clone(),
    ...
    let recaptured = client
        .reconcile()
        .await
        .expect("the next healthy reconciliation recaptures the retained parent");
    assert_eq!(recaptured.redriven, 0, "{recaptured:#?}");
    let successor = client
        .decide_tick_round(ProbeFacts {
            probes: vec![(fed(1), dying_probe(100)), (fed(2), healthy_probe(0))],
            ...
            now_ms: 3,
            price_routes: false,
            blocked: recaptured.blocked,
            admission_snapshot: recaptured.admission_snapshot,
        })
        .await
        .expect("retained parent plans a higher-occurrence child");
    let successor_child = successor
        .replacement
        .as_ref()
        .expect("replacement at the higher occurrence")
        .fresh
        .clone();
    assert_eq!(successor_child.occurrence, Occurrence(82));
    let committed = client
        .commit_tick(
            successor,
            BTreeMap::from([(fed(1), Msat(1_000)), (fed(2), Msat(0))]),
            client
                .issue_balance_facts_token()
                .await
                .expect("fresh facts for higher occurrence"),
        )
        .await
        .expect("a higher occurrence exchanges the retained parent");
    assert_eq!(committed.accepted, vec![successor_child.idempotency_key]);
    assert_eq!(
        journal
            .get(&parent.idempotency_key)
            .await
...
/// Exchange confirmation asks whether the ATTEMPTED parent acquired its child. A parent that was
/// itself a replacement carries a reverse `A -> B` predecessor sidecar, so a dual-key read returns
/// that predecessor when confirming `B -> C`. Accepting it as this exchange's outcome would turn a
/// definitely uncommitted fault into a marker-retaining ambiguity that poisons tick authority. The
/// daemon path must read only `B`'s canonical successor — the actor-side parity of
/// `runtime::tests::standalone_replacement_confirmation_ignores_a_middle_parents_predecessor`.
#[tokio::test]
async fn confirmed_uncommitted_exchange_ignores_a_middle_parents_predecessor() {
    let (service, journal) = fixture(Arc::new(SlowExecutor::default())).await;
    let client = service.client();
    let (grandparent, grandparent_evidence, middle_cap) =
        qualifying_marker(fed(1), fed(2), Occurrence(860));
    journal
        .upsert(&grandparent)
        .await
        .expect("seed the first marked parent");
    let middle_key = evacuation_key(fed(1), fed(2), Occurrence(861));
    let middle_decision = AllocatorDecision {
        action: Action::Evacuate {
            from: fed(1),
            to: fed(2),
            amount: Msat(100),
            fee_cap: middle_cap.at(Msat(100)),
            gateway: None,
            fee_cap_components: Some(middle_cap),
        },
        reason: ReasonCode::ShutdownNotice,
        occurrence: Occurrence(861),
        idempotency_key: middle_key.clone(),
    };
    assert!(
        journal
            .replace_marked_evacuation(
                &grandparent.idempotency_key,
                grandparent.attempt,
                &grandparent_evidence,
                &middle_decision,
                2,
                &grandparent,
            )
            .await
            .expect("A -> B exchange"),
        "the middle parent must exist with a durable predecessor relation"
    );

    // Mark B itself: the same structural refusal, now measured against the cap B was created with.
    let new_cap = EvacFeeCap {
        base_msat: Msat(30),
        bps: 0,
    };
    let middle_evidence = EvacuationRefusalEvidence {
        cap_components: middle_cap,
        requested_net: Msat(100),
        source_spendable: Msat(200),
        low: EvacuationQuoteSample {
            delivered_net: Msat(10),
            total_fee: Msat(21),
            fee_cap: middle_cap.at(Msat(10)),
        },
        high: EvacuationQuoteSample {
            delivered_net: Msat(100),
            total_fee: Msat(21),
        ...
        diagnostic: "controlled qualifying structural refusal on a replaced parent".to_owned(),
        measured_at_ms: 2,
    };
    assert!(wallet_core::evacuation_cap_qualifies_replacement(
        &middle_evidence,
    ...
    let mut middle = journal
        .get(&middle_key)
        .await
        .expect("read the middle parent")
        .expect("the middle parent exists");
    middle.evacuation_refusal = Some(middle_evidence.clone());
    journal
        .upsert(&middle)
        .await
        .expect("mark the middle parent");
    let mut policy = client.get_policy().await.expect("read policy");
    policy.per_fed_cap = Msat(1_000);
    policy.spending_target = Msat(100);
    policy.standby_target = Msat(100);
    ...
    client
        .put_policy(policy)
        .await
        .expect("install the qualifying cap");
    let round = client
        .decide_tick_round(ProbeFacts {
            probes: vec![(fed(1), dying_probe(100)), (fed(2), healthy_probe(0))],
            occurrence: Occurrence(862),
            now_ms: 3,
            price_routes: false,
            blocked: wallet_core::GoalBlockers::default(),
            admission_snapshot: client.issue_tick_plan_token().await.expect("plan token"),
        ...
        .await
        .expect("the replaced parent plans its own replacement");
    let child_key = {
        let replacement = round.replacement.as_ref().expect("B -> C replacement");
        assert_eq!(replacement.old_key, middle_key, "{round:#?}");
        replacement.fresh.idempotency_key.clone()
    };
    let facts = client
        .issue_balance_facts_token()
        .await
        .expect("fresh facts before the injected error");
    journal.fail_before_next_evacuation_replacement_for_test();
    let report = client
        .commit_tick(
            round,
        ...
        .await
        .expect("a middle parent's predecessor cannot make B -> C ambiguous");
    assert!(report.accepted.is_empty(), "{report:#?}");
    assert_eq!(report.refused.len(), 1, "{report:#?}");
    assert_eq!(report.refused[0].key, child_key);
    assert_eq!(report.refused[0].reason, RefuseReason::Conflict);
    let middle_after = journal
        .get(&middle_key)
        .await
        .expect("read the middle parent after confirmation")
        .expect("the middle parent survives an uncommitted exchange");
    assert_eq!(middle_after.status, IntentStatus::Pending);
    assert_eq!(
        middle_after.evacuation_refusal,
        Some(middle_evidence),
        "a definite uncommitted outcome retains the exact marker"
    );
    assert_eq!(
        journal.get(&child_key).await.expect("read the child key"),
        None,
        "uncommitted confirmation invents no child"
    );
    assert!(
        journal
            .evacuation_canonical_successor(&middle_key)
            .await
            .expect("read the middle parent's canonical successor")
            .is_none(),
        "the attempted exchange wrote no successor relation"
    );
    assert_eq!(
        journal
            .evacuation_supersession(&middle_key)
            .await
            .expect("read the middle parent's durable relation")
            .expect("A -> B remains")
            .old_key,
        grandparent.idempotency_key,
        "the predecessor remains audit history, not this exchange's outcome"
    );
    assert!(
        client.issue_tick_plan_token().await.is_ok(),
        "a definite outcome must not poison goal admission authority"
    );
    assert!(
        client.issue_balance_facts_token().await.is_ok(),
        "a definite outcome must not poison balance-fact authority"
    );
    service.shutdown().await.expect("shutdown");
}

#[tokio::test]
async fn ambiguous_exchange_confirmation_retains_marker_and_poisons_goal_and_balance_authority() {
    let executor = Arc::new(SlowExecutor::default());
    let (service, journal) = fixture(executor.clone()).await;
    let client = service.client();
    let (parent, evidence, round, balances) =
        qualifying_replacement_round(&client, &journal, Occurrence(90), Occurrence(91)).await;
    let child_key = round
        .replacement
        .as_ref()
        .expect("replacement")
    ...
    let facts = client
        .issue_balance_facts_token()
        .await
        .expect("fresh facts before injected ambiguity");
    journal.fail_after_next_evacuation_replacement_ambiguously_for_test();
    let error = client
        .commit_tick(round, balances, facts)
        .await
        .expect_err("mixed exchange confirmation must fail closed");
    assert!(
        matches!(&error, ServiceError::Storage(message) if message.contains("outcome is ambiguous")),
        "{error:?}"
    );
    let old = journal
        .get(&parent.idempotency_key)
        .await
        .expect("read mixed parent")
        .expect("mixed parent remains");
    assert_eq!(
        old.evacuation_refusal,
        Some(evidence.clone()),
        "ambiguous confirmation must not consume the durable marker evidence"
    );
    assert_eq!(
        old.status,
        IntentStatus::Failed,
        "exchange may have committed"
    );
    assert!(
        journal
            .get(&child_key)
            .await
            .expect("read possible child")
            .is_some(),
        "mixed durable child remains for recovery, not a guessed cleanup"
    );
    assert_eq!(
        registry_size(&client).await,
        0,
        "no unconfirmed child owner starts"
    );
    assert!(
        client.issue_tick_plan_token().await.is_err(),
        "goal admission authority is poisoned"
    );
    assert!(
        client.issue_balance_facts_token().await.is_err(),
        "balance-fact authority is poisoned"
    );
    let child = journal
        .get(&child_key)
        .await
        .expect("read possible child before isolated marker recovery")
        .expect("mixed durable child remains for recovery");
    journal
        .set_status(
            &child_key,
            child.attempt,
            IntentStatus::Failed,
            Some("isolate poisoned marker recovery"),
        )
        .await
        .expect("terminalize unrelated ambiguous child for isolated marker recovery");
    let mut recovery_only_marker = parent.clone();
    recovery_only_marker.idempotency_key =
        IdempotencyKey("evac:poisoned-recovery-only-marker".to_owned());
    recovery_only_marker.status = IntentStatus::Pending;
    journal
        .upsert(&recovery_only_marker)
        .await
        .expect("seed an exact planner marker after planning poison");
    let recovery = client
        .reconcile_recovery_only_cycle()
        .await
        .expect("poisoned recovery-only scan still recovers unrelated durable ownership");
    assert_eq!(
        recovery.redriven, 0,
        "poisoned recovery-only mode must not attach a driver to the exact marker"
    );
    assert_eq!(
        registry_size(&client).await,
        0,
        "poisoned recovery-only mode starts no marker driver"
    );
    assert_eq!(
        executor.calls.load(Ordering::SeqCst),
        0,
        "the isolated recovery-only scan did not invoke the executor"
    );
    assert_eq!(
        journal
            .get(&recovery_only_marker.idempotency_key)
            .await
            .expect("read poisoned recovery-only marker"),
        Some(recovery_only_marker.clone()),
        "poisoned recovery-only mode preserves exact marker evidence and starts no driver for it"
    );
    let after_reconcile = journal
        .get(&parent.idempotency_key)
        .await
        .expect("read marker after poisoned reconcile")
        .expect("mixed parent remains after poisoned reconcile");
    assert_eq!(
        after_reconcile.evacuation_refusal,
        Some(evidence),
        "a poisoned reconcile must neither recapture-and-release nor clear retained ambiguity evidence"
    );
    service.shutdown().await.expect("shutdown");
}

/// An ambiguous exchange fails closed on PLANNING, and deliberately not on durable recovery.
///
/// The commit itself installs no owner for a child it could not prove. But the exchange may in
/// fact have committed — exactly the case here, where the sidecar is corrupted away AFTER a
/// successful write — and ADR-0029 forbids stranding a dying federation's balance. So the next
/// ownership-recovery pass must still rehydrate that durable child and drive it, while both
/// poisoned planning authorities keep refusing to mint a fresh round until restart.
#[tokio::test]
async fn ambiguous_exchange_recovery_drives_the_committed_child_while_planning_stays_poisoned() {
    let (service, journal) = fixture(Arc::new(SlowExecutor::default())).await;
    let client = service.client();
    let (parent, evidence, round, balances) =
        qualifying_replacement_round(&client, &journal, Occurrence(92), Occurrence(93)).await;
    let child_key = round
        .replacement
        .as_ref()
        .expect("replacement")
    ...
    assert_eq!(
        registry_size(&client).await,
        0,
        "the unproven commit installs no owner itself"
    );
    // This ambiguity is an audit-read fault over an exchange that DID commit.
    let old = journal
        .get(&parent.idempotency_key)
        .await
        .expect("read mixed parent")
    ...
    assert_eq!(
        old.evacuation_refusal,
        Some(evidence),
        "the retained marker is recovery evidence, not a live planning handoff"
    );
    assert_eq!(
        journal
            .get(&child_key)
    ...
    let report = client
        .reconcile_durable()
        .await
        .expect("durable ownership recovery is not gated on planning authority");
    assert_eq!(
        report.redriven, 1,
        "the committed child regains exactly one durable owner: {report:#?}"
    );
    wait_for_registry(&client, 1).await;

    // Recovery of the money is automatic; recovery of PLANNING authority is not.
    assert!(
        client.issue_tick_plan_token().await.is_err(),
        "goal admission authority is still poisoned"
    );
    assert!(
        client.issue_balance_facts_token().await.is_err(),
        "balance-fact authority is still poisoned"
    );
    assert!(
        matches!(
            client.reconcile().await,
            Err(ServiceError::Storage(message)) if message.contains("poisoned")
        ),
        "a scheduler reconcile still refuses to mint fresh authority"
    );
    assert_eq!(
        registry_size(&client).await,
        1,
        "the recovered owner survives the refused scheduler pass"
    );
    service.shutdown().await.expect("shutdown");
}
