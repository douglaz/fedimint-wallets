...
| `HTTP_PROXY` / `HTTPS_PROXY` / `ALL_PROXY` (and lowercase), `NO_PROXY` | `walletd`, `wallet-cli` (`wallet-web` has no outgoing HTTP client) | every `reqwest` client — the CLI's daemon client (`WalletdClient::resolve`), the gateway `routing_info` client (`MultiClient::new`), the Observer client (`ObserverSource::new`) | honoured by `reqwest` 0.12's default system-proxy support; no builder calls `no_proxy()`. With a proxy set and `NO_PROXY` not covering loopback, client-mode `wallet-cli` sends its plaintext bearer-authenticated daemon request through the proxy (`SEC-3`) |
| `TOKIO_WORKER_THREADS` | all three (`#[tokio::main]`, multi-thread runtime) | worker-thread count of the pinned Tokio runtime | read by Tokio, not the wallet: `0` or a non-numeric value **panics at startup** (exit 101, stderr only) rather than falling back |
...
procedure exists, and the preimage is not one — and MUST run in this order
