...
occurrence overflow and discovery or deadline storage faults), `corrupt_federation_registry`
(with the skipped-row count), `partial_federation_view`. `automation_ready` on `/v1/health` is
its negation (`API-16`).
